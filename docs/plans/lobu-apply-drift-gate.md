...
- `apply-cmd.ts`: after `computeDiff`, if blocking drift exists or un-gated deletes exist → render blocked report, POST the blocked deployment, exit 1. `--confirm-deletes <token>`: recompute, verify the full blocking set (any mismatch rejects the entire token), print decoded list, then **execute the complete recomputed plan** — the token authorizes its destructive subset; ordinary config updates in the same plan run as normal config-wins work. Config-expressed deletes proceed under `--yes` as today. **POST the summary on every fully-succeeded run including all-noop** — a **`--only` run advances the baseline per its executed family only** (decision #2). `lobu apply --bootstrap` snapshots current remote into `attribution` with an **empty `owned` set**.
