 ...
 * automation source queries which omit an `id` column.
 *
 * Bug being pinned: automation-mode content aggregation (queryContentData in
...
describe("queryProjectsIdColumn", () => {
	...
	it("tolerates automation template placeholders ({{entityId}}, {{query.x}})", () => {
		expect(
			queryProjectsIdColumn(
