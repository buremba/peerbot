/**
 * Compact behavior automation contracts retained from the deleted broad suite.
 *
 * These are high-value queue/lifecycle boundaries: scheduled behaviors should
...
import {
	dispatchPendingBehaviorRuns,
	materializeDueBehaviorRuns,
	reconcileBehaviorRuns,
	runBehaviorAutomationTick,
	sweepStaleBehaviorRuns,
...
async function createAutomatedBehavior() {
	const sql = getTestDb();
	const dbClient = sql as unknown as DbClient;
	const workspace = await TestWorkspace.create({
		name: "Behavior Automation Contract Org",
	});
...
describe("behavior automation contract", () => {
	beforeEach(async () => {
		await cleanupTestDatabase();
	});

	it("materializes one scheduled behavior run and dedupes concurrent ticks", async () => {
		const { sql, behaviorId, agent, workspace } = await createAutomatedBehavior();

		const [resultA, resultB] = await Promise.all([
			materializeDueBehaviorRuns({} as Env),
			materializeDueBehaviorRuns({} as Env),
		]);
    ...
    `;
		...
		expect(Number(payload.behavior_id)).toBe(behaviorId);
		expect(payload.agent_id).toBe(agent.agentId);
		expect(payload.dispatch_source).toBe("scheduled");
	});

	it("reconciles a queued behavior run when a correlated window already exists", async () => {
		const { sql, dbClient, workspace, behaviorId, agent } =
			await createAutomatedBehavior();
		...
		const windowRootId = await createCanvasWindow({
			...
			runMetadata: { source: "external", behavior_run_id: queued.runId },
		});

		const result = await dispatchPendingBehaviorRuns({
			db: dbClient,
			runIds: [queued.runId],
		});
		const [run] = await sql`
	...
	it("completes a queued behavior run from complete_window provenance", async () => {
		const { sql, dbClient, workspace, api, entityId, behaviorId, agent } =
			await createAutomatedBehavior();
    ...
    `;
		...
		const completion = (await api.behaviors.completeWindow({
			...
			extracted_data: { summary: "Automated behavior summary" },
			run_metadata: {
				executor: "lobu-agent",
				agent_id: agent.agentId,
	...
	it("skips behavior runs pinned to a device worker (#802)", async () => {
		const { sql, dbClient, workspace, behaviorId, agent } =
			await createAutomatedBehavior();
		...
		// when the behavior is bound to a Mac/CLI device. Until that lands the
    ...
    `;
		...
		// queueAndDispatchBehaviorRun helper hits this branch when a behavior run
		// is manually triggered.
		const targeted = await dispatchPendingBehaviorRuns({
			db: dbClient,
			runIds: [queued.runId],
		});
	...
	it("paginates behavior reads by cursor and completes from multiple page tokens", async () => {
		const { sql, workspace, api, entityId, behaviorId } =
			await createAutomatedBehavior();
		...
		for (let i = 0; i < 5; i++) {
			events.push(
				await createTestEvent({
					...
					content: `Paginated behavior content ${i}`,
	...
	it("links the exact signed content IDs without re-running behavior sources", async () => {
		const { sql, workspace, api, entityId, behaviorId } =
			await createAutomatedBehavior();
		...
		const event = await createTestEvent({
			entity_id: entityId,
			organization_id: workspace.org.id,
			content: "Content returned to the behavior worker.",
			occurred_at: new Date(Date.now() - 60 * 60 * 1000),
		});
	...
	// #798 — device-pinned behavior execution end-to-end:
	//
	//   behavior.device_worker_id set
	//     → materializeDueBehaviorRuns persists the pin into approved_input
	...
	describe("device-pinned execution (#798)", () => {
        ...
        VALUES ('user-behavior-pin', 'device-pin-1', 'macos', ${sql.json({})}, 'My Mac')
        RETURNING id
      `;
      ...
      `;
			...
			const result = await materializeDueBehaviorRuns({} as Env);
      ...
      `;
		...
		// behavior agents. The dispatcher's POST is only an exit report that
		...
		it("exit report acks an MCP-completed run and stamps device provenance", async () => {
			const { sql, dbClient, workspace, api, behaviorId, agent } =
				await createAutomatedBehavior();
			const granularity = inferBehaviorGranularityFromSchedule("0 9 * * *");
			const { windowStart, windowEnd } = await computePendingWindow(
				dbClient,
      ...
      `;
			...
			const content = (await api.knowledge.read({ behavior_id: behaviorId })) as {
				window_token: string;
			};
			const completion = (await api.behaviors.completeWindow({
				behavior_id: String(behaviorId),
      ...
      `;
        ...
        WHERE id = ${behaviorId}
      `;
			expect(behavior.last_fired_at).not.toBeNull();
      ...
      `;
			...
			const completion = (await api.behaviors.completeWindow({
				...
				run_metadata: { source: "device_worker", behavior_run_id: queued.runId },
			})) as {
				window_id: number;
        ...
        SELECT reaction_type, tool_name FROM behavior_reactions
        WHERE window_id = ${completion.window_id}
      `;
		...
		it("fires the reaction script on a zero-content replace_existing (head_superseded gate)", async () => {
			const { api, behaviorId } = await createAutomatedBehavior();
			await api.behaviors.setReactionScript({
				behavior_id: String(behaviorId),
				reaction_script: "export default async function reaction() { return; }",
			});
		...
		// the failure mode that masked the broken Reddit behavior for a week.
      ...
      `;
			...
			// Schedule must still advance so the behavior doesn't re-fire forever.
			const [after] =
				await sql`SELECT next_run_at FROM behaviors WHERE id = ${behaviorId}`;
      ...
      `;
		...
		it("complete-behavior endpoint refuses non-Behavior run types", async () => {
			const sql = getTestDb();
			const { workspace } = await createAutomatedBehavior();
      ...
      `;
		...
		// the scheduler re-fires the behavior every tick forever. This run never
		// calls complete_window, so once the finalize_nudges budget is spent the
		// report fails it — next_run_at must still move.
		it("advances behaviors.next_run_at on a terminal exit report", async () => {
			const { sql, dbClient, workspace, behaviorId, agent } =
				await createAutomatedBehavior();
			const granularity = inferBehaviorGranularityFromSchedule("0 9 * * *");
			const { windowStart, windowEnd } = await computePendingWindow(
        ...
        SELECT next_run_at FROM behaviors WHERE id = ${behaviorId}
      `;
			const beforeNextRun = before.next_run_at as Date | string | null;
      ...
      `;
			...
			// 10min in the past by createAutomatedBehavior).
			const beforeMs = beforeNextRun ? new Date(beforeNextRun).getTime() : 0;
      ...
      `;
			...
			// Behavior run pinned to worker B (via approved_input.device_worker_id).
			const granularity = inferBehaviorGranularityFromSchedule("0 9 * * *");
			const { windowStart, windowEnd } = await computePendingWindow(
				dbClient,
				behaviorId,
	...
	// Regression: an active behavior run carrying a `dispatched_message_id` must
	...
	// `malformed array literal: "<uuid>"`. Because `behavior-automation` (every
	// tick) AND `check-stalled-executions` both call reconcileBehaviorRuns, a single
	// such run wedged BOTH jobs — behaviors stopped firing in prod for 12 days (run
	...
	describe("headless terminal delivery resolves behavior runs on first claim (no SSE owner)", () => {
		// Regression for the owner-gate false-negative: behavior dispatch never
		...
		// exhaust 30 retries, dead-letter, and skip resolveBehaviorRunsByMessageIds
		...
		async function makeRunningBehaviorRun(messageId: string) {
			const { sql, dbClient, workspace, behaviorId, agent } =
				await createAutomatedBehavior();
			const granularity = inferBehaviorGranularityFromSchedule("0 9 * * *");
			const { windowStart, windowEnd } = await computePendingWindow(
      ...
      `;
		...
		function terminalPayload(messageId: string, runId: number, error?: string) {
			return {
				messageId,
				channelId: `api_behavior_${runId}`,
				conversationId: `api_behavior_${runId}`,
				userId: `behavior-${runId}`,
				teamId: "api",
				timestamp: Date.now(),
		...
		it("re-dispatches for a finalize nudge when the reply produced no window (first miss)", async () => {
			...
			const { sql, runId } = await makeRunningBehaviorRun(messageId);
      ...
      `;
		...
		it("respects a per-behavior finalize_nudges=0 override (fails immediately, no re-dispatch)", async () => {
			// The per-behavior budget (execution_config.finalize_nudges) overrides the
			...
			const messageId = randomUUID();
			const { sql, runId, behaviorId } = await makeRunningBehaviorRun(messageId);
			await sql`
        UPDATE behaviors
      ...
      `;
		...
		it("completes the run immediately when a window exists", async () => {
			const messageId = randomUUID();
			const { sql, workspace, runId, behaviorId, windowStart, windowEnd } =
				await makeRunningBehaviorRun(messageId);
			...
			const windowRootId = await createCanvasWindow({
				...
				runMetadata: { behavior_run_id: runId },
			});
    ...
    `;
		...
		const result = await reconcileBehaviorRuns(getDb());
		expect(result.reconciled).toBe(0);
	});

	describe("behavior-automation tick orchestration", () => {
		it("recovers an orphaned event-delivery claim for durable retry", async () => {
			const { sql, behaviorId } = await createAutomatedBehavior();
			const materialized = await materializeDueBehaviorRuns({} as Env);
			...
			const result = await runBehaviorAutomationTick({} as Env);
		...
		// separate due behavior.
		it("survives a wedging in-flight run and still materializes other due behaviors", async () => {
			const { sql, dbClient, workspace, behaviorId, agent } =
				await createAutomatedBehavior();

			// Behavior A: a stuck active run with a dispatched_message_id and no window —
			...
			const stuck = await createBehaviorRun({
				organizationId: workspace.org.id,
				behaviorId,
				agentId: agent.agentId,
				windowStart: windowStart.toISOString(),
      ...
      `;
			...
			// Behavior B: due, no active run, valid agent → must materialize this tick.
			const entityB = await createTestEntity({
				name: "Tick Entity B",
				organization_id: workspace.org.id,
				created_by: workspace.users.owner.id,
			});
			const behaviorB = (await workspace.owner.behaviors.create({
				entity_id: entityB.id,
				slug: "tick-behavior-b",
				name: "Tick Behavior B",
				prompt: "Summarize content for the bound entities.",
				triggers: [
					{
						kind: "schedule",
			...
			const behaviorBId = Number(behaviorB.behavior_id);
			await sql`UPDATE behaviors SET next_run_at = NOW() - INTERVAL '10 minutes' WHERE id = ${behaviorBId}`;
			...
			// Behavior B materialized despite behavior A's stuck run.
			expect(result.runsCreated).toBeGreaterThanOrEqual(1);
			const [runB] = await sql`
        SELECT status FROM runs
        WHERE behavior_id = ${behaviorBId} AND run_type = 'behavior'
      `;
			expect(runB).toBeDefined();
		});

		// Item 1: a behavior whose assigned agent no longer exists (and isn't device
		...
		it("does not schedule a behavior whose agent does not exist; counts it unrunnable", async () => {
			const { sql, behaviorId } = await createAutomatedBehavior();

			// Point the behavior at a non-existent agent, no device pin, due now.
			await sql`
        UPDATE behaviors
      ...
      `;
			...
			const runs = await sql`
        SELECT id FROM runs WHERE behavior_id = ${behaviorId} AND run_type = 'behavior'
      `;
			expect(runs).toHaveLength(0);
		});
	});

	describe("sweepStaleBehaviorRuns liveness reaping", () => {
		it("finalizes a stale scheduled pending run and advances its behavior schedule", async () => {
			const { sql, behaviorId } = await createAutomatedBehavior();
			const materialized = await materializeDueBehaviorRuns({} as Env);
			expect(materialized.runsCreated).toBe(1);
      ...
      `;
			...
			const [behavior] =
				await sql`SELECT next_run_at FROM behaviors WHERE id = ${behaviorId}`;
			expect(new Date(behavior.next_run_at as string).getTime()).toBeGreaterThan(
				Date.now()
			);

			const retry = await materializeDueBehaviorRuns({} as Env);
		...
		it("leaves a fresh scheduled pending run available for dispatch", async () => {
			...
			const { timedOut } = await sweepStaleBehaviorRuns(sql);
      ...
      `;
		...
		it("does not expire a stale manually triggered pending run", async () => {
			const { sql, behaviorId } = await createAutomatedBehavior();
			await materializeDueBehaviorRuns({} as Env);
			await sql`
        UPDATE runs
        SET created_at = NOW() - INTERVAL '3 hours',
            approved_input = jsonb_set(
      ...
      `;
			...
			const [behavior] =
				await sql`SELECT next_run_at FROM behaviors WHERE id = ${behaviorId}`;
			expect(new Date(behavior.next_run_at as string).getTime()).toBeLessThan(
				Date.now()
			);
		});
      ...
      `;
		...
		// Seed a `running` behavior run with controlled claim/heartbeat ages.
		...
		async function seedRunningBehaviorRun(opts: {
			claimedAgo: string;
			heartbeatAgo?: string;
		}) {
			const { sql, dbClient, workspace, behaviorId, agent } =
      ...
      `;
		...
		it("reaps a heartbeating run that went silent past the heartbeat window", async () => {
			...
			const { sql, runId } = await seedRunningBehaviorRun({
				claimedAgo: "1 hour",
...
describe("canvas-on-events window completion", () => {
	...
	it("emits a canvas_state ROOT event on window completion", async () => {
		const { sql, behaviorId } = await completeOnce({
      ...
      WHERE semantic_type = 'canvas_state'
        AND (metadata->>'behavior_id')::bigint = ${behaviorId}
    `;
	...
	it("completes with a PAT/device client_id that is not an oauth_clients row", async () => {
		...
		const { sql, behaviorId, completion } = await completeOnce({
	...
	it("a same-period completion without replace_existing is an idempotent no-op", async () => {
		...
		const { sql, api, behaviorId, windowStart, windowEnd, completion } =
			await completeOnce({
				extracted_data: { summary: "v1" },
			});
	...
	it("replace_existing supersedes the head, keeping the root id stable", async () => {
		const { sql, api, behaviorId, windowStart, windowEnd, workspace } =
			await completeOnce({
				extracted_data: { summary: "v1" },
			});
	...
	it("replace_existing re-links exactly the new content set (no stale links)", async () => {
		...
		const links = await sql`
      SELECT event_id FROM behavior_window_events WHERE window_id = ${first.window_id}
    `;
	...
	// Manual triggers complete through the same advanceBehaviorSchedule as
	...
	// already current.
	it("advanceBehaviorSchedule is idempotent when next_run_at is already the upcoming tick", async () => {
		const { sql, dbClient, behaviorId } = await createAutomatedBehavior();

		const upcomingTick = nextRunAt("0 9 * * *", new Date());
		await sql`
      UPDATE behaviors SET next_run_at = ${upcomingTick}::timestamptz
      WHERE id = ${behaviorId}
    `;

		await advanceBehaviorSchedule(dbClient, behaviorId);
		await advanceBehaviorSchedule(dbClient, behaviorId);
