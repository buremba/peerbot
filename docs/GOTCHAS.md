...
**Multi-workspace OAuth grants have a default-off fleet cutover.** The additive grant columns and grant-aware server/Owletto code must be deployed everywhere with `LOBU_OAUTH_MULTI_WORKSPACE_GRANTS` absent (or any value other than the exact string `1`). In that state, new backends accept legacy one-workspace consent, enabled-only picker payloads with more than one workspace are rejected, and the UI capability marker is omitted so new UI served against an old/disabled backend submits only `organization_id`. After every externally reachable backend pod is on the grant-aware build and old pods are drained, enable with Helm value `app.env.LOBU_OAUTH_MULTI_WORKSPACE_GRANTS: "1"`. The flag gates new multi-workspace approval, token exchange, and refresh; it deliberately does not disable grant verification/enforcement.

Rollback is also two-phase. First remove/set the flag away from `1` across the new fleet and wait for the multi-workspace UI marker to disappear. Before routing traffic to any old binary, revoke active access/refresh tokens and pending authorization/device codes whose explicit `granted_organization_ids` contains more than one workspace, and remove their MCP sessions so clients re-authorize as a singleton. Merely turning the issuance flag off does not invalidate already-issued access tokens, and an old verifier would otherwise treat them with legacy all-membership semantics.
...
**WhatsApp Browser keeps the internal key `whatsapp.local` during the transport cutover.** Preserving the key reuses the existing connection, single `messages` feed, and `origin_id` dedupe history. The Chrome extension always advertises the manifest as connector inventory, but advertises its dedicated `browser.whatsapp` capability only after the user grants WhatsApp Web access. The latest device manifest, capability set, and heartbeat are the canonical facts; `ready`, `setup_required`, and `device_offline` are derived at read time and must not be stored as a second setup-status silo. The feed declares both `sync` and `read`: sync builds local searchable history, while an explicit source read queries the device without persisting its result. A manifest-backed connector is claimable only by a device advertising the exact winning manifest and required capability, not by every device sharing a generic browser capability.
