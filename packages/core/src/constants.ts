...
/**
 * Runtime gate for multi-chunk embedding writes. Contract-phase code (per-model
 * delete, 3-col PK) ships with this release; chunking stays off until every pod
 * runs it, then flip LOBU_EMBEDDING_CHUNKING=1 globally.
 */
export function isEmbeddingChunkingEnabled(): boolean {
  const v = process.env.LOBU_EMBEDDING_CHUNKING?.trim().toLowerCase();
