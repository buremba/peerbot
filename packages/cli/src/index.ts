...
`
  ...
  withCommonOpts(
    program
      ...
      .option(
        "--feed <key>",
        "Local feed to create so the connection syncs (repeatable)",
    ...
    async (
      ...
      await connectCommand(connector, {
        org: options.org,
        context: options.context,
        cloudContext: options.cloudContext,
        slug: options.slug,
        name: options.name,
        feed: options.feed,
        // Commander stores `--no-open` as `open: false`.
