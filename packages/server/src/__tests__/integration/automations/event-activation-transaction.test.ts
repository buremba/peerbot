...
describe("Automation activation transactionality", () => {
	beforeAll(async () => {
	 ...
	 * Replica safety of createAutomationEventRun depends on its per-Automation
	 ...
	 */
	it("holds the per-Automation advisory lock across the coalesce read and never drops the signal", async () => {
		...
		const matches = await findMatchingAutomationActivations(org.id, {
			...baseSignal,
			delivery_id: "event:tx:1",
		});
		expect(matches).toHaveLength(1);

		const [seeded] = await queueAutomationActivations({
			matches,
		...
		const queueing = queueAutomationActivations({
			matches,
