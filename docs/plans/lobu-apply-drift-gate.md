...
Status: **v1 shipped in PR #2663** (token + adopt codegen deferred, `--bootstrap` dropped — see "What actually shipped") · Owner: @buremba · Builds on `lobu-apply.md` (v1 merged) · Related to `lobu-pull.md` (shares the attribution record) · Reviewed against pi second-opinion: 7 rounds hardened an earlier, stronger fail-closed contract; **revised to a narrowed contract** (coordination-design-brake); rounds 8–12 on the narrowed contract each closed 1–2 blockers (baseline evolution, `--only` scope, mixed-plan confirm, token→full-plan binding, write-only `reaction_script`, mandatory index) — re-review pending

## Goal

Make `lobu apply` stop silently destroying **un-declared** remote state, while keeping the config authoritative for everything it declares. Before PR #2663, a config-managed org with `prune: true` (e.g. `examples/personal-agent`, org `buremba`) did two harmful things to manual work done in the web UI:

- a UI edit to a declared definition (adding `x-lobu.role` to `task.status`) produced an `update` row and apply overwrote it;
- a UI-created definition (`contact`) produced a `delete` row and apply proposed deleting it.

The contract, **narrowed**:

1. **Config-declared state converges, config wins** (Terraform semantics) — including a UI edit that lands in the plan-to-execute window. This is the declared owner's semantics, not an accident.
2. **Un-declared remote state is never clobbered or auto-deleted.** A remote-only *change to a declared definition* blocks whenever a recorded baseline shows the remote moved. A remote-only *definition* blocks only under `prune` — that is the only mode in which apply would delete it. Off prune it is informational drift: nothing would be deleted, so blocking protects nothing and merely stalls the rest of the plan. Today a blocked definition is resolved by adopting it into config or deleting it through the UI/API; adopt codegen and named deletes remain deferred.
3. **Never auto-delete a UI-created definition.** Delete eligibility is config-expressed only.
4. A future **reconciler agent** gets clean, safe options: adopt (autonomous, git-visible) or delete/revert (approval-gated, named).
...
```
   ...
   desired ≠ remote AND remote ≠ attribution  → remote moved / both moved → BLOCK (manual resolution today)
   -- remote-only definitions: the rows below apply UNDER PRUNE only.
   -- Off prune the apply cannot delete a definition, so every remote-only
   -- definition is informational drift regardless of ownership.
   ...
   remote-only definition, NOT in an `owned` set
                                              → UI-created → BLOCK (manual adopt/delete; named delete deferred)
   no recorded baseline (never applied / legacy manifest)
                                               → declared definitions use two-way convergence
                                               → remote-only drift is informational without prune
                                               → remote-only prune candidates BLOCK (no ownership proof)
```

Once a baseline is recorded, convergence happens **only when the remote is untouched** (`remote == attribution`). If both config and a concurrent UI edit moved the same field, that field blocks — never let config silently win over a user's edit. Before any baseline is recorded, declared definitions use the ordinary two-way diff so legacy orgs can converge; ownership-dependent deletion remains fail-closed. Creates converge naturally. The config-expressed delete tier is conditional: a definition edited in the UI after the baseline is a both-moved block, not a delete.
...
- **`owned`** — the set of **kind-qualified definition incarnation identities** `{ kind, id }` (kind ∈ entity-type | relationship-type | automation | connector-definition; ids from independent domains: `entity_types.id`, `entity_relationship_types.id`, `automation_id`, `connector_definitions.id`) **this config actually applied** (delete-eligible). Qualifying by kind is load-bearing: the ID domains are independent, so an unqualified set can collide across kinds (e.g. `entity_types.id = 5` and `automation_id = 5`) and misclassify a UI-created definition as owned. Storing `{ kind, id }`, never slugs or connector key+version, is what makes delete eligibility sound: a config-expressed delete that succeeds and then has its summary POST fail leaves the identity in `owned`; if the user re-creates the same slug (or re-installs the same connector key/version — uninstall archives the row, so a reinstall is a **new `connector_definitions.id`**) in the UI, the new row has a **new id** and is therefore **not owned** — it blocks as drift instead of being auto-deleted under `--yes`.

Full and `--only memory` applies record the memory/Automation baseline. `--only agents` preserves the prior baseline and leaves it absent when none exists.

## Background before PR #2663

This section records the implementation starting point; its line references are historical.
...
1. **Fail-closed drift gate on un-declared state.** If the plan contains blocking drift, apply prints the report, exits non-zero (`2`, the CLI's `ValidationError` code), and mutates nothing — including under `--yes`. Blocking is scoped to what the apply could actually destroy: a remote-moved field on a declared definition (recorded baseline), or a remote-only definition **under prune**. `--yes` never destroys un-declared state.
2. **Three-way attribution via the baseline.** `noop` when desired == remote; **converge only when `remote == attribution`**; **block whenever `remote ≠ attribution AND desired ≠ remote`** (remote-move or both-moved). The compare uses the **`attribution` snapshot (effective remote after the apply)**, never the `desired` state — unmanaged facets (`eventKinds`/`viewTemplate`) live remotely but not in `desired`, so `desired`-based attribution would permanently block config-expressed deletes. **Without a recorded baseline, declared definitions use the ordinary two-way diff, and no definition is delete-eligible because ownership cannot be proven.** Remote-only definitions block under prune and are informational off prune, with or without a baseline — the rule keys on `prune`, not on recordedness, matching how connector definitions have always worked. The **baseline advances only on a fully-succeeded run, including all-noop runs** (apply must POST the summary on noops). **Scope-preserving:** a **`--only agents|memory`** run advances the baseline **only for its executed family**, carrying forward the unexecuted families from the prior baseline (a per-family composite); a first `--only agents` run leaves the unexecuted baseline absent. The baseline separates **`attribution`** from **`owned`**: full and memory-scoped applies populate both. **`lobu rollback` bypasses the drift gate** — it is an explicit operator restore of a recorded `desired` state and force-converges to it (config-wins), then re-records the baseline. It achieves this by calling `computeDiff` with **no baseline at all**, so the no-baseline path is load-bearing production code (rollback and creates), not a legacy shim to be migrated away; a legacy org leaves it after one successful full or memory-scoped apply, including an all-noop one.
3. **Field-level scope: memory schema + Automations.** Entity types (properties, required, name, description, backing, metrics, eventKinds, viewTemplate, resolutionPolicy), relationship types, and Automations get per-field attribution; a remote-moved field is a blocking item. **Write-only Automation `reaction_script` is exempt** — the server never returns it, `diffAutomation` always re-pushes it (idempotent), so it is never attributed or blocked. **Connector definitions are excluded from field-level attribution** (their source is re-pushed idempotently — never field-blocked) **but included in the owned-based delete classification** (decision #4), so a UI-created connector is never auto-deleted. Everything else (agents, settings, connections, feeds, auth profiles, providers) keeps today's semantics — converge what's declared, non-blocking drift notes. Rationale: the idempotent/declared paths there emit perpetual "update" rows by design (connector-definition re-push, BYO connection `config` always-changed, write-only provider keys).
4. **Two delete tiers.**
   ...
   - **Un-declared** (remote-only, never in an `owned` set): UI-created. **Never a delete row.** Blocking drift **under prune**, informational drift **off prune** — off prune the apply could not delete it anyway, so blocking would protect nothing and only stall the rest of the plan. The prune-scoped block is resolved today by manual adopt or UI/API deletion. Named delete remains deferred.
...
- **Concurrent applies are not serialized.** Two applies on the same org can interleave; this plan assumes a single reconciler path (one Automation → one apply). A future need for concurrent applies gets its own admission design.
...
- **A first baseline grants attribution only.** The apply that records an org's first baseline never makes pre-existing UI-created definitions delete-eligible: `owned` holds only the incarnations that apply itself created.

## Phasing

### v1 (this plan) — deterministic core

- Field-level attribution + drift gate: block + non-zero exit on blocking drift; blocked report with remote values. Adopt snippets and the named-delete token are deferred.
- Config-expressed deletes ride `--yes` (shipped); `--confirm-deletes <token|slug...>` is deferred.
- Baseline advances on every fully-succeeded run including noop.
- Blocked applies POST a `blocked` deployment with candidates; the optional token is not minted yet.
- No agent loop: the event is emitted and a human (or existing agent tooling) resolves it.

### v2 — reconciler loop

- A `reconcile-config` Automation watches `blocked` deployments, reads candidates + token, and per item: adopt (edit `lobu.config.ts`, open a PR), delete/revert (issue the named delete → `pending_approval`), or leave.
- Agent PR path for adopt (requires a repo-write/PR tool). Until then, adoption is a manual config edit using the remote values in the blocked report.

## v1 work breakdown — 3 PRs

**Planned dependency order**: PR-1 (server) first; PR-2 and PR-3 on top. The implementation ultimately shipped as one branch.

### What actually shipped (PR #2663 — PR-1 + PR-2 + PR-3 combined)

The three PRs landed as one branch. **Shipped**: `blocked` deployment status + `candidates` payload, `GET /deployments/latest` + its partial index, the connector incarnation id in the catalog, the `{ attribution, owned }` baseline in the manifest, three-way field attribution for entity/relationship types and Automations, the owned-based two-tier delete classification (including connectors), the fail-closed drift gate (exit `2`, nothing mutated, blocked deployment POSTed), the `✖ BLOCKED` report, baseline advance on all-noop and `--only`-scoped runs, and the rollback bypass + baseline re-record.
...
- ~~`--bootstrap`~~ **dropped, not deferred.** It solved a problem that does not exist: an org with no recorded baseline gets one from its next successful full or memory-scoped apply, including an all-noop run. A first `--only agents` apply intentionally leaves the memory/Automation baseline absent because it does not execute that family. Verified end-to-end — an org forced back to an attribution-less manifest recorded `attribution` + `owned` after a single `0 create, 0 update, 1 noop` apply. Snapshotting remote into `attribution` with an empty `owned` set would produce the same classifications the unrecorded path already produces, so the flag would add a command surface and buy nothing. If a bootstrap-shaped need resurfaces, the useful version is the opposite direction — *adopt* remote definitions into `lobu.config.ts` so they become declared — which is the adopt-snippet codegen below, not a baseline snapshot.
...
The token/flag bullets in PR-2 and PR-3 below therefore describe the follow-up, not this branch. Paths and line numbers in the work breakdown are historical design references.
...
- `diff.ts`: replace whole-`properties` deepEqual in `diffEntityType` with per-field comparison (including `resolutionPolicy`); add the three-way attribution pass against the **`attribution` snapshot** (converge only when `remote == attribution`; block when `remote ≠ attribution AND desired ≠ remote`; use two-way convergence before a baseline is recorded). Blocking-drift row: `{ kind, id, field?, verb: "drift", blocking: true, adoptSnippet, remoteChange }`. Delete rows only for definitions whose **incarnation ID is in the baseline's `owned` set** and whose remote still matches `attribution`; under prune, remote-only-absent-from-owned and edited-after-baseline become blocking drift; off prune every remote-only definition is informational drift. Preserve org-ownership and `$`-system guards.
...
3. **No recorded baseline never grants delete ownership** — declared definitions use two-way convergence, but remote-only prune candidates block.
...
7. **`event_kinds` / `viewTemplate`**: don't regress the declared-vs-omitted semantics in `diffEntityType`.
8. **Idempotent re-push rows excluded from blocking** (connector defs, BYO connections, provider keys, **write-only Automation `reaction_script`** — always-repush, never attributed, or they block forever).
...
10. **A first baseline never grants ownership of pre-existing remote-only definitions**: delete classification consults `owned`, never raw manifest/`desired` membership.
...
Token and adopt-codegen cases below are deferred acceptance criteria; the remaining cases describe the shipped gate.

### Unit

- `diff-baseline.test.ts`: three-way attribution table — (attribution, desired, remote) × outcome, for entity-type fields (including `resolutionPolicy`), relationship-type fields, Automation fields, remote-only-in-owned, remote-only-in-owned-but-edited-after-baseline → block, remote-only-not-in-owned, no recorded baseline → two-way for declared definitions, remote-only definitions blocking only under prune, both-moved → block, **an unchanged unmanaged facet (`eventKinds`/`viewTemplate`) does not block a config-expressed delete, but a post-baseline edit to one does**, **a same-slug UI recreation (new incarnation ID) after a delete-with-lost-summary is never auto-deleted — it blocks as drift**. **Connector cases**: UI-created/unowned → never deleted; previously-applied-and-unchanged → deleted under prune; post-baseline edit → block; a first baseline grants no connector ownership; a referenced connector (`liveConnectorKeys`) is never deleted; **uninstall → failed summary POST → same-key/same-version UI reinstall yields a new `connector_definitions.id` that is unowned and blocks, never auto-deleted**; **cross-kind ID collisions (e.g. `entity_types.id = automation_id`) never make a UI-created definition delete-eligible (`{ kind, id }` ownership)**; same-slug relationship-type recreation after a delete-with-lost-summary blocks as drift.
- `deployment.test.ts`: token mint/verify round-trip; tampered/stale token (any item changed) rejects the whole token; cross-org/origin token rejected; **a config update introduced after token mint rejects the token (hash covers the entire executable plan), so nothing executes without fresh confirmation**; mixed-plan confirm (blocking drift + ordinary config update) executes the full recomputed plan and finalizes the baseline to match; **write-only `reaction_script` never blocks (always-repush preserved)**; **rollback force-converges to `desired` and is never blocked by drift attribution**; baseline (noop run advances, **a `--only` run advances only its executed family and carries forward the rest**, partial_failure does not, missing baseline uses two-way convergence without granting ownership).
...
10. No-baseline org: one ordinary `lobu apply --yes` records the first baseline (attribution + owned) even when the plan is all-noop; immediately after, a `prune: true` apply must NOT delete the pre-existing remote-only definitions (attribution ≠ ownership) — they block instead.
 ...
 (server, (CLI     (CLI
  small)   core,    report,
     ...
     E2E (this session)
            │
            ▼
   v2: reconcile-config Automation + agent PR path
```

The planned stack collapsed into combined PR #2663. v2 (the agent loop) depends on the live blocked-deployment event plus the deferred token/adopt flow.
...
- ❌ Reconcile-config Automation / agent PR automation (v2).
