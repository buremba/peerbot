...
## Track A — stabilize PR #1533 (this branch, non-breaking, immediate)
- **A1** Fix keyingConfig camelCase→snake_case in `map-config.ts` + test. *(no migration)* ← STARTING HERE
- **A2** Full `json_template` removal (code reads/writes; column drop deferred to contract). 0 prod usage.
- **A3** Two-phase: **remove** the `extraction_schema` DROP migration from this branch; defer to a contract release.
- **A4** Restore classifier `source_path` lint vs the *derived* entity-type schema (recover the deleted validator's loud-fail).
- **A5 (DECISION)** **Keep inline `extraction_schema` as the escape hatch** for reaction-handoff watchers; do NOT remove until Track C lands. → #1533 becomes: keyingConfig (fixed) + json_template removal + lint + entity-typed derive (already on main). Non-breaking.
