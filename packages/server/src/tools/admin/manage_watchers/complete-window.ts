...
export async function handleCompleteWindow(
  ...
  const result = await sql.begin(async (tx) => {
    ...
    if (tokenWindowId) {
      ...
      if (existingWindow.length > 0) {
        if (args.replace_existing) {
          ...
          // Remove the membership edges directly.
    ...
    if (batchContentIds.length > 0) {
      // Write the membership edges directly to event_edges (org + watcher_id_hint
      // resolved from this window's watcher). event_edges.id is GENERATED; the unique
      // (parent,child,edge_type) dedups vs any old-pod trigger writes.
