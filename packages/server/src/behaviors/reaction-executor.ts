 ...
 * Reaction Executor
 *
 * Executes compiled behavior reaction scripts inside the shared `runScript`
...
export async function executeReaction(options: ExecuteReactionOptions): Promise<{
  ...
  // Reactions are scoped to the behavior's own workspace — they have no user
  ...
  const reactionCtx = {
    ...
    scopedToOrg: true,
    allowCrossOrg: false,
    // The reaction IS this behavior acting autonomously. Stamping the behavior id
    // here makes EVERY gated write it performs (connector ops, entity mutations,
    // behavior edits) resolve the behavior's owning agent and evaluate in
    // autonomous mode — the script cannot dodge its agent's envelope by omitting
    // an explicit `behavior_source`. `source: 'behavior-run'` marks the turn
    // autonomous even for surfaces that read only sourceContext.
    actingBehaviorId: context.window.behavior_id,
    ...
    sourceContext: { source: 'behavior-run' as const },
  };
 ...
 * its handler. This is how the behavior's extraction contract is derived from
