/**
 * Integration test for P2 phase 1: promoting keyed behavior-window rows into
 * real child entities.
 *
 * complete_window computes stable keys (keying_config) and then promotes each
 * keyed row into a child entity under the behavior's bound parent, keyed by an
 * entity_identities `behavior_key` claim (the idempotency lock). Origin
 * provenance (window_id / stable_key / behavior_id) is stamped onto the child
 ...
 * The behavior's extraction contract is DERIVED from this (an array of these
 * records at `keying_config.entity_path`), never authored on the behavior.
 */
...
async function setupKeyedBehavior() {
  const sql = getTestDb();
  ...
  await sql`UPDATE behaviors SET next_run_at = NOW() - INTERVAL '10 minutes' WHERE id = ${behaviorId}`;
...
/**
 * Queue + claim a running behavior run for the behavior's pending window so a
 * completion lands on the run-driven path (which makes the SAME window
 * reusable for an idempotent replay).
 */
async function queueRunningRun(ctx: Awaited<ReturnType<typeof setupKeyedBehavior>>) {
...
async function readWindowToken(
  ctx: Awaited<ReturnType<typeof setupKeyedBehavior>>
): Promise<string> {
...
async function completeWithToken(
  ...
  const completion = (await ctx.api.behaviors.completeWindow({
    behavior_id: String(ctx.behaviorId),
    window_token: windowToken,
    extracted_data: extractedData,
    run_metadata: { behavior_run_id: runId },
...
describe('complete_window promotes keyed rows into entities (P2 phase 1)', () => {
  ...
  it('creates a child entity per keyed row, with origin window provenance in its metadata', async () => {
    const ctx = await setupKeyedBehavior();
    ...
    const identities = await sql`
      ...
      WHERE ei.organization_id = ${workspace.org.id}
        AND ei.namespace = 'behavior_key'
      ORDER BY ei.identifier
    `;
    expect(identities.map((r) => String(r.identifier))).toEqual([
      `${ctx.behaviorId}::performance::slow-loading`,
      `${ctx.behaviorId}::stability::app-crashes`,
    ]);
    ...
    // creates the per-behavior canvas entity as a child of the same parent
    // (canvas-on-events, metadata.source='behavior_canvas'), so scope the
    ...
    const promoted = childTypes.filter((r) => r.source !== 'behavior_canvas');
    expect(promoted).toHaveLength(2);
    expect(promoted.every((r) => String(r.slug) === 'topic')).toBe(true);
    const canvasChildren = childTypes.filter((r) => r.source === 'behavior_canvas');
    ...
    const childMeta = await sql`
      ...
      WHERE ei.organization_id = ${workspace.org.id} AND ei.namespace = 'behavior_key'
      ORDER BY ei.identifier
    `;
    ...
    for (const row of childMeta) {
      ...
      expect(Number(md.behavior_id)).toBe(ctx.behaviorId);
    }
    ...
    expect(csMeta.kind).toBe('behavior_change_set');
  ...
  it("a create=deny policy on the behavior's OWNING AGENT blocks its promotions", async () => {
    // The v1.1 fix: a behavior is its agent's autonomous mode, so the agent's own
    // envelope binds the behavior. Pin entity create=deny to THIS behavior's agent;
    // the promotion must create nothing. Before the fix the gate resolved the
    // behavior as principal `behavior:<id>` (agentId null), so this agent-scoped
    // deny never matched and the rows were created under the looser org default.
    const ctx = await setupKeyedBehavior();
    const { sql, workspace, behaviorId, parentEntityId, agent } = ctx;
    ...
    // No `topic` entities were promoted — the agent's deny bound the behavior.
    const promoted = await sql<{ c: number }>`
      SELECT COUNT(*)::int AS c
      FROM entity_identities
      WHERE organization_id = ${workspace.org.id}
        AND namespace = 'behavior_key'
        AND identifier LIKE ${`${behaviorId}::%`}
    `;
  ...
  it('is idempotent across a same-window replay — no duplicate entities', async () => {
    const ctx = await setupKeyedBehavior();
    const { sql, workspace, behaviorId, parentEntityId } = ctx;
    ...
    const entitiesAfterFirst = await sql`
      ...
      WHERE organization_id = ${workspace.org.id} AND namespace = 'behavior_key'
      ORDER BY entity_id
    `;
    ...
    // 'behavior_canvas'), never mint a second canvas entity.
    ...
    const canvasCount = await sql`
      ...
      WHERE parent_id = ${parentEntityId}
        AND organization_id = ${workspace.org.id}
        AND metadata->>'source' = 'behavior_canvas'
    `;
  ...
  it('syncs extracted fields into entities and respects a human-owned field on re-run, queuing an approval', async () => {
    ...
    const appCrashesId = `${ctx.behaviorId}::stability::app-crashes`;
    const [created] = await sql`
      SELECT e.id, e.metadata, e.field_controls
      FROM entities e JOIN entity_identities ei ON ei.entity_id = e.id
      WHERE ei.namespace = 'behavior_key' AND ei.identifier = ${appCrashesId}
    `;
    ...
    // Slice 2 (match): the behavior does NOT overwrite the human-owned value.
    ...
    // Still owned — an approved behavior value remains human-owned, not behavior-writable.
  ...
  it('approving a STALE proposal does not clobber a value the human moved after it was queued', async () => {
    const ctx = await setupKeyedBehavior();
    const { sql, workspace, behaviorId } = ctx;
    ...
    const appCrashesId = `${behaviorId}::stability::app-crashes`;
    const [created] = await sql`
    ...
    // Run 2: behavior proposes 'critical' against the 'high' snapshot → pending approval.
    await ctx.api.behaviors.completeWindow({
      behavior_id: String(behaviorId),
      window_token: token,
      run_metadata: { behavior_run_id: runId },
  ...
  it('list_promoted returns the behavior promoted entities with field ownership + provenance', async () => {
    const ctx = await setupKeyedBehavior();
    const { sql, workspace, behaviorId } = ctx;
  ...
  it('approve (affirm_fields) locks a value as-is so a later behavior change is blocked', async () => {
    const ctx = await setupKeyedBehavior();
    const { sql, workspace, behaviorId } = ctx;
    ...
    // Run 1 seeds `severity: 'low'` (behavior-owned, no field_controls yet).
    await ctx.api.behaviors.completeWindow({
      behavior_id: String(behaviorId),
      window_token: token,
      run_metadata: { behavior_run_id: runId },
    ...
    // affirmed the field, the behavior must be BLOCKED and queue an approval —
  ...
  it('disambiguates a slug that collides with a pre-existing sibling — window is NOT poison-pilled', async () => {
    ...
    // Both keyed rows promoted: two behavior_key identities exist.
    const identities = await sql`
  ...
  it("groups a run's proposals by window and approves them all in one batch", async () => {
    ...
    // Run 2 (same window): the behavior proposes a new severity for BOTH — each is
  ...
  it("reject_batch cancels a run's proposals and records the reason for revision", async () => {
    ...
    const [row] = await sql`
      ...
      WHERE ei.organization_id = ${workspace.org.id} AND ei.namespace = 'behavior_key' LIMIT 1
    `;
    await workspace.owner.entities.update({
      entity_id: Number(row.entity_id),
      metadata: { severity: 'high' },
    ...
    // SAME channel getRecentFeedbackSummary reads to feed the behavior's next run
    // (the revision loop). Keyed to the behavior, field_path='$batch_reject'.
    const feedback = await sql`
      SELECT metadata FROM current_event_records
      WHERE organization_id = ${workspace.org.id}
        AND semantic_type = 'correction'
        AND (metadata->>'kind') = 'behavior_batch_reject'
    `;
