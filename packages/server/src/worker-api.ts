...
export async function completeWorkerJob(c: Context<{ Bindings: Env }>) {
  try {
    ...
    if (updatedRuns.length === 0) {
      ...
      // claimant. authorizeRunForWorker gated ownership in `user` worker-auth
      // mode, so this is almost always the timeout race. Skip all feed/auth
      // bookkeeping and return an idempotent already-finalized response.
      logger.info(
