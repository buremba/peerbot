/**
 * Shared entity field-merge primitive for the behavior<->human feedback loop.
 *
 ...
 * This is the SINGLE write path for both human edits and behavior promotion:
 *   - source='human'  : writes every changed field AND marks it owned (note/set_by/set_at).
 *   - source='behavior': writes only fields that are NOT human-owned; owned fields are
...
export type FieldWriteSource = "human" | "behavior";
...
export interface FieldMergeResult {
	...
	/** Owned fields a behavior tried to change — NOT written; surface as an approval. */
	 ...
	 *  is now claimed so a behavior can't silently overwrite it. */
	affirmed: string[];
...
export function computeFieldMerge(args: {
	...
	for (const [field, value] of Object.entries(fields)) {
		...
		// A behavior must never overwrite a human-owned field — propose instead.
		// Policy may also require approval for fields that are not human-owned yet.
		if (source === "behavior" && (owned || requireApproval.has(field))) {
 ...
 * can emit the audit event (human path) or the approval interactions (behavior path).
 */
