...
import { backfillReactionInputSchema } from '../../../automations/backfill-reaction-input-schema';
...
async function seedReactionAutomation(workspace: TestWorkspace, suffix: string, script: string) {
...
describe('backfillReactionInputSchema', () => {
  ...
  it('fills a NULL reaction_input_schema from the stored script (dry-run writes nothing)', async () => {
    ...
    const automationId = await seedReactionAutomation(workspace, 'fill', REACTION_WITH_INPUT);

    // set_reaction_script already populated it; NULL it to simulate a pre-feature row.
    await sql`UPDATE automations SET reaction_input_schema = NULL WHERE id = ${automationId}`;
    ...
    const [afterDry] =
      await sql`SELECT reaction_input_schema FROM automations WHERE id = ${automationId}`;
  ...
  it('leaves a reaction with no `input` export as NULL (counted, never written)', async () => {
    ...
    const automationId = await seedReactionAutomation(workspace, 'noinput', REACTION_NO_INPUT);
    await sql`UPDATE automations SET reaction_input_schema = NULL WHERE id = ${automationId}`;
    ...
    const [after] = await sql`SELECT reaction_input_schema FROM automations WHERE id = ${automationId}`;
