...
describe("automation source refs", () => {
	it("parses event-backed refs", () => {
		expect(parseAutomationSourceRef("@feed:support")).toEqual({
			type: "feed",
			value: "support",
		});
		expect(parseAutomationSourceRef("@connection:gmail")).toEqual({
			type: "connection",
			value: "gmail",
		});
		expect(parseAutomationSourceRef("@connector:slack")).toEqual({
			type: "connector",
			value: "slack",
		});
		expect(parseAutomationSourceRef("@channel:#support")).toEqual({
			type: "channel",
			value: "#support",
		});
	});
	...
	it("parses entity and metric refs as context sources", () => {
		const entity = parseAutomationSourceRef("@entity:customer");
		const metric = parseAutomationSourceRef("@metric:customer.retention");
		...
		expect(automationSourceKindForRef(entity)).toBe("entity");
		expect(automationSourceKindForRef(metric)).toBe("metric");
	});

	it("leaves raw SQL alone", () => {
		expect(parseAutomationSourceRef("SELECT id FROM events")).toBeNull();
		expect(validateAutomationSourceRef("content", "SELECT id FROM events")).toBeNull();
	});

	it("rejects unsupported or unsafe refs", () => {
		expect(() => parseAutomationSourceRef("@metric:customer")).toThrow(/metric/i);
		expect(() => parseAutomationSourceRef("@entity:bad.slug")).toThrow(/slug/i);
		expect(() => parseAutomationSourceRef("@feed:support';DROP")).toThrow(
			/unsupported characters/i,
		);
		expect(() => validateAutomationSourceRef("x", "@unknown:y")).toThrow(
			/unsupported/i,
		);
	});
});
...
describe("automationSourcesFromPrompt", () => {
	...
	it("de-dupes by query and makes duplicate names unique", () => {
		const prompt =
			...
			"@[feed:k2:Issues](/o/b)";
		const out = automationSourcesFromPrompt(prompt);
		expect(out).toEqual([
	...
	it("returns [] for a prompt with no source tokens", () => {
		expect(automationSourcesFromPrompt("just plain instructions")).toEqual([]);
	});
});
...
describe("normalizeAutomationSources source.context classification", () => {
	// A no-ref SQL source never touches the DB in normalizeAutomationSources, so a
	...
	it("classifies a plain SQL source as event content (id must be an events.id)", async () => {
		const [normalized] = await normalizeAutomationSources(sql, "org", [
