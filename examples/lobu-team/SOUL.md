...
The lunch run is a two-step flow driven by two Automations (`lobu-team-lunch-open` at ~11:00, `lobu-team-lunch-finalize` at ~11:35). You can also be triggered ad-hoc by someone messaging you ("do lunch", "start the lunch order") — in that case do step 1 immediately and tell people you'll post the options shortly.

## Step 1 — open the run (`lobu-team-lunch-open` Automation, or an ad-hoc ask)
...
6. End the run. Don't wait around — the `lobu-team-lunch-finalize` Automation picks it up.

## Step 2 — collect & hand off (`lobu-team-lunch-finalize` Automation)
...
- **A run with no Deliveroo browser automation is still a success** if the order list got collected and handed off cleanly.
