...
- **Types own schema + template.** Two first-class type registries — `entity_types` and a new
  first-class `event_types` (lifted out of `entity_types.event_kinds`). Behaviors own neither.
...
- **Track A** — behavior authors no schema/template; rendering is the type's job. The reason this
  plan exists.
...
- **C0.1** Define the **config-change event** shape (`semantic_type='config_change'`):
  `{ target_kind (entity_type|event_type|template|classifier|behavior), target_id, field_path,
...
- **C0.3** Make runtime config writers emit config-change events: `manage_view_templates`,
  `manage_entity_schema`, `manage_classifiers`, behavior CRUD. (Audit lands on the spine — the
...
- **A0.4** Retire the version tables (`view_template_versions`, `behavior_versions`) once the config
