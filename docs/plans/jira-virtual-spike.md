...
> **Status:** Spike complete (offline + code-path measurement). Live Atlassian optional via `LOBU_SPIKE_JIRA=1`.
...
| Corpus | **None** in the chat product — source of truth stays Atlassian |
...
| Reads | Virtual feed: `query()` / `search()` → `/search/jql` (shipped) |
...
| --- | --- | --- | --- | --- | --- | --- |
| S1 | Structured ops JQL (“open P1s in SUPP”) | **Yes** | Yes (index lag) | **Yes** | Yes (stale) | `buildJiraJql` + unit pushdown; optional live |
...
| S4 | Comment-heavy thread summarize | **Yes** (`getJiraIssue`) | Yes | **Gap** — description only in row | Thin / raw webhook | `fields=` excludes `comment` (spike test) |
...
| S9 | Transition / comment with approval | **Yes** | Varies | **No** | **No** | `actions: {}`, no write scopes (spike test) |
...
## Connector auth / tenant survey (all shipped connectors)
...
| **jira** | **oauth 3LO** | **`cloud_id` (Atlassian Cloud site)** | **Yes — only one** |
...
1. OAuth callback → `resolveJiraCloudSite(accessToken)` → stamp
   `connection.config.cloud_id|site_url|site_name` + `external_tenant_id`.
...
```bash
# Offline matrix + JQL scenarios + gap assertions
cd .claude/worktrees/jira-virtual-feed
bun test packages/connectors/src/__tests__/jira-spike-matrix.test.ts

# Optional live JQL against a real site
LOBU_SPIKE_JIRA=1 \
  JIRA_SPIKE_TOKEN=<oauth_access_token> \
  JIRA_SPIKE_CLOUD_ID=<cloud_id> \
  JIRA_SPIKE_JQL='project = SUPP AND statusCategory != Done ORDER BY updated DESC' \
  bun test packages/connectors/src/__tests__/jira-spike-matrix.test.ts
```

Also run (already green on this branch):
...
These need `LOBU_SPIKE_JIRA=1` + a sandbox site; offline spike still closed the architectural decisions.
