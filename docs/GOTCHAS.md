...
**WhatsApp Browser keeps the internal key `whatsapp.local` during the transport cutover.** The Chrome extension may advertise that exact key with either the legacy `browser.scripting` capability or the dedicated `browser.whatsapp` activation capability; preserving it reuses the existing connection, single `messages` feed, and `origin_id` dedupe history. That feed declares both `sync` and `read`: sync builds local searchable history, while an explicit source read queries the device without persisting its result. A manifest-backed connector is claimable only by a device advertising the exact winning manifest, not by every device sharing its capability.
