# Behavior / Entity Consolidation — Implementation Plan
...
1. **Source-ranked projection** — fold append-only events → `(target, key) → value`, resolving
   ...
   (`entity_field_state` trigger, `event_classifications` read-CTE, behavior-`correction` DISTINCT ON),
...
- keyingConfig config-API is camelCase but server reads snake_case; nothing translates →
  config-authored entity-typed behaviors silently become untyped (`map-config.ts:606` vs `behavior-extraction-schema.ts:98`). **Bug (pi blocker).**
- Promotion is create-once / no-op-on-match (`promote-keyed-entities.ts:266`) — can't evolve entities.
- Behavior version *history* is dead (`behavior-detail.tsx:96` `includeVersions:false`; `upgrade` unreachable + buggy at `version-actions.ts:243`); only run-snapshot + group-sharing are load-bearing.
...
- **No compat shims / escape hatches. Prefer one-time MANUAL data migration.** When removing a
  ...
  behavior/event rows) into the clean model — do NOT keep the old code path alive for compat. Keep the
...
- **A5 (RESOLVED — remove, migrate data manually)** Inline `extraction_schema` is transitional debt, not
  ...
  migration of the 3 live reaction-handoff behaviors into the entity / reaction-reads-entity model:
...
- **C3** Migrate reaction behaviors to entity output + reaction-reads-entity (lunch done; HN next).
...
- **D1** Collapse `submit_feedback` window-`correction` onto the event-level field/label edit; behavior feedback reads projected state. One correction concept.

## Track E — versioning collapse (depends on nothing; mirrors classifier P4)
- **E1** Inline behavior config onto group-root row; snapshot resolved config into `runs.approved_input`;
  config-change events for audit; drop `behavior_versions`/`current_version_id`/`version`/`change_notes`/`source_behavior_id`; keep `behavior_group_id`. Replace `create_from_version` (init-from-org clone). Delete dead+buggy `upgrade`/`get_versions`.
