...
entity, changing a schema, adding an entity type, creating an agent/automation/schedule,
...
**which principal** (a specific agent or automation) is acting. Humans are governed by
...
4. **Agents & automations ARE principals** — policy may target `principal_kind`
   (agent|automation) + optional `principal_id`. NULL = any. This is the newly-requested
   capability: "automation #6 may auto-create person entities; every other agent needs
   approval."
...
```
id, organization_id,
resource_class     text NOT NULL,   -- entity | entity_type | agent | automation | schedule
...
principal_kind     text NULL,        -- agent | automation ; NULL = any
principal_id       text NULL,        -- specific agent/automation id ; NULL = any of kind
...
restrictive-wins. Prove red→fix→green: automation #N auto-allowed while other agents gated;
...
- **v1.2 — more classes**: automation · schedule · feed · classifier.
...
per-*entity*, `entity-field-approval.ts:303`, not batch grouping). An automation window
creating 100 entities ⇒ 100 cards. Unusable at automation scale.
...
The `runs.window_id`/`runs.automation_id` COLUMNS exist (baseline.sql:1868-1890) but proposals
...
- **Revision loop = reject-with-reason re-dispatches the agent (DECIDED).** Rejecting a
  batch or an item with a reason re-runs the automation/agent with that reason as context;
...
lands, BEFORE pushing automations-at-scale on customers (100 cards makes the feature
unusable for exactly automations' main use case).
...
1. **Migration is NOT deploy-window safe — needs two-phase (CRITICAL).** Migrations run in
   ...
   agent/automation entity write throws (`loadCandidatePolicies` errors; only
...
3. **§6b window_id claim was WRONG — corrected.** `manage_operations.ts:695` is
   `trackAutomationReaction` for connector-action runs (writes `automation_reactions.window_id`), a
   ...
   `runs.window_id` nor `runs.automation_id` columns (which EXIST, baseline.sql:1868-1890 —
   automation_id currently lives only in `action_input`). **DECIDED: thread windowId through the
...
## 6c. Automation-run change-set is FIRST-CLASS — diff ≠ approval (DECIDED)
**Correction to §6b framing.** The diff of "what an automation run changed" is a property of
...
**Two layers:**
1. **Run change-set (always visible, any policy outcome).** Every automation/agent run
   records its create/update/delete change-set, grouped by `automation_run_id`/`window_id`,
   viewable as a diff on the RUN itself — whether auto-applied, approval-gated, or denied.
   This is observability ("what did automation #6 do at 3pm?"), independent of gating.
...
**Already partially exists** (grounds this, not net-new): `automation_run_id` threaded through
`complete_window` (`manage_automations/complete-window.ts`);
`promoted-entities-recap.tsx` + `automation-summary-view.tsx` in owletto render a run recap.
...
(`automation_run_id`), not invented inside the approval flow.
...
1. **Principal identity mapping — SPECIFY exactly.** Not just `agentId`. Define
   principal resolution for every context: agent run → `(agent, agentId)`; automation run →
   `(automation, automationId)`; system/automation token (no agent id) → `(agent, NULL)`; user →
   never a policy principal. Today `automationId` is *attribution*, not policy identity —
...
- **#2 automation_source principal spoof (high, FIXED):** caller-supplied `automation_source`
  could reclassify an agent as `automation:<id>` to dodge its agent policy.
...
- **#9** thread `window_id` through `manage_entity` automation writes (not only promotion).
