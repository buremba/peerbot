...
--
-- TWO-PHASE INVARIANT (until those readers move to `kind`): no writer sets `kind`
-- independently today — this migration only backfills it. The phase that starts
-- creating virtual/streaming feeds MUST keep `virtual` consistent
...
-- scheduler (which still gates on `f.virtual`) never queues a non-collected feed.
--
