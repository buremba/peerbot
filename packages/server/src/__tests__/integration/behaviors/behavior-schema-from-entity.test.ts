/**
 * Integration test: a behavior derives its extraction schema from its target
 * entity type's metadata_schema (consolidation — "schema lives on the entity
 * type"). When a behavior names keying_config.entity_type, complete_window
 ...
 *   2. deriveBehaviorExtractionSchema resolves a real entity type's schema.
 *   3. complete_window REJECTS extracted data that violates the entity type's
 *      schema and ACCEPTS data that conforms — with no inline behavior schema.
 */
...
import {
  deriveBehaviorExtractionSchema,
  wrapMetadataSchemaAtPath,
} from '../../../utils/behavior-extraction-schema';
...
async function setupEntityTypedBehavior() {
  const sql = getTestDb();
  ...
  // The 'topic' type carries the metadata_schema that becomes the behavior's
  ...
  // NB: no inline behavior schema — the behavior relies on the entity type's schema.
  const behavior = (await workspace.owner.behaviors.create({
    entity_id: parentEntity.id,
    slug: 'schema-behavior',
...
type Ctx = Awaited<ReturnType<typeof setupEntityTypedBehavior>>;
...
describe('complete_window derives its schema from the entity type', () => {
  ...
  it('resolves a real entity type metadata_schema into an extraction schema', async () => {
    const ctx = await setupEntityTypedBehavior();
    const derived = (await deriveBehaviorExtractionSchema(
      ctx.dbClient,
  ...
  it('ACCEPTS extracted data that conforms to the entity type schema (no inline schema)', async () => {
    ...
    const promoted = await ctx.sql`
      ...
      WHERE ei.organization_id = ${ctx.workspace.org.id} AND ei.namespace = 'behavior_key'
    `;
