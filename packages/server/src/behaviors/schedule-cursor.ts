...
/**
 * Move a behavior's `next_run_at` forward to the next cron tick after now.
 *
 ...
 * behaviors (manual-only) are no-ops.
 *
 ...
 *   throwing would roll the caller back forever. Worse, `dispatchBehaviorRun`
 *   calls `failBehaviorRun` from inside its own `catch`, and
 *   `dispatchPendingBehaviorRuns` has no per-run guard — a throw there escapes
 ...
 */
export async function advanceBehaviorSchedule(
	sql: DbClient,
  ...
  `;
	...
	try {
		...
		logger.error(
			{ error, behaviorId, schedule, timezone },
			"[behaviors] Behavior schedule is unparseable — parking it (next_run_at = NULL). " +
 ...
 * (`failBehaviorRun`), turn resolution (`resolveBehaviorRunsByMessageIds`), and
...
export async function advanceScheduleAfterTerminalFailure(
	...
	await advanceBehaviorSchedule(sql, behaviorId);
}
