...
| `EGRESS_JUDGE_MODEL` | Optional | Deployment-wide **fallback** for the optional LLM egress judge. An agent's guardrail names its own model, so leaving this unset is the stricter choice — the config API then requires every judge guardrail to name one. Whichever names it, the ref must be a qualified `<provider>/<model>` (e.g. `openai/gpt-4o-mini`) naming a provider this deployment holds a **system key** for in its environment, and that speaks the OpenAI-compatible protocol; Anthropic-protocol providers (`claude/...`) are not eligible. A guardrail naming an ineligible model is rejected at save time; an ineligible value **here** instead fails closed at runtime and denies every judged egress. |
