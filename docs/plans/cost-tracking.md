# PLAN — Cost & Usage Tracking for Agents / Automations / Users / Orgs
...
- Breakdowns surfaced: **per-org, per-agent, per-automation, per-user/per-thread** (all four).
...
- Hard-cap dimensions in v1: **org + automation** (most-restrictive configured cap wins). Agent/user get visibility + soft alerts, not blocking, in v1.
- Over-budget UX: **block + tell the user** — headless/automation runs terminalize with a "budget exceeded" error event; chat turns get a user-facing over-budget reply via the existing terminal-delivery path.
...
- Identity: `org_id`, `agent_id`, `conversation_id`, `user_id`, `automation_id`, `run_id`, `source` (chat/automation/scheduled/connector-repair/internal), `model`, `provider`.
- Tokens (distinct buckets — **never one `input_tokens`**): `input`, `output`, `cache_read`, `cache_write`. (Cache-read ≈0.1× input on Anthropic; collapsing overstates cache-heavy automation loops ~5–10×.)
...
- Keys: `UNIQUE(run_id)` (or reuse the snapshot key `UNIQUE(org, agent, conv, run_id)`), `ON CONFLICT DO NOTHING` for ret/duplicate safety. Indexes on `(org_id, occurred_at)` and each capped dimension `(automation_id, occurred_at)`.
...
cost_budgets(organization_id, scope, scope_id, window,
  ...
  -- scope: 'org' | 'automation' (scope_id null = org-wide); window: 'daily'|'weekly'|'monthly'
...
2. **CLI — `lobu.config.ts` → `lobu apply`.** Code-first orgs declare budgets/overrides in config; `apply` reconciles them into the two tables via the existing desired-state path (same as agents/Automations/connections). **No MCP tool** (decided).

**Automation overlap:** the existing `automations.execution_config.max_budget_usd` is a *per-run* ceiling (claude-CLI flag), kept as-is. The new *sliding-window* Automation cap lives in `cost_budgets` (`scope='automation'`); the Automation panel surfaces/links the budget editor — one budgets model, no two-places-to-set-a-cap.
...
- **Device-worker CLI automation runs** (Owletto local claude/codex) report back **no cost** today (`run-lifecycle.ts:604-622`) and have no `session.jsonl` on the gateway side. Cloud Lobu worker runs are covered; the device-CLI path needs separate handling (claude/codex CLIs can emit cost JSON) — scope as a follow-up, flag in v1 that device-CLI automation spend is not captured.
...
- **VERIFIED this session** (installed + ran the npm package): it's a genuine JS port (ESM+CJS, typed, only dep `yargs`, `node>=20`) — *not* a Python wrapper. It imports and computes in Node, and its catalog carries the real tiers, e.g. `claude-sonnet-4-5 input_mtok = {base:3, tiers:[{start:200000, price:6}]}`. Measured a 250k-in/50k-out run: **pi-ai flat `$1.50` vs genai-prices tiered `$2.625` — a 43% undercharge** that pi-ai would silently bake in. Below 200k both return identical `$0.30`, so the swap only changes large-context runs (exactly the automation/agent loops we care about). It's `v0.0.x` (young) → **pin the exact version + vendor the catalog + wrap behind our own `priceUsage()` adapter** so it's swappable.
...
- **Per-automation** spend over time on the automation detail/tab.
...
- **Gate location:** immediately AFTER a successful claim, BEFORE spawn — at `RunsQueue.claimOne()` (`runs-queue.ts`, chat/agent) and `poll.ts` claim (connector/automation/auth); chat ingest at the `message-consumer.ts` admission point (where the input-guardrail trip already terminalizes).
...
- **First concrete cap dimension:** keep `automations.execution_config.max_budget_usd` as the existing per-run device-CLI ceiling (`AutomationExecutionConfigSchema`; `AutomationDispatcher.buildArguments`) and add the first sliding-window Automation cap in `cost_budgets` (`scope='automation'`).
...
4. **PR-3 (showback UI):** per-org chip + dashboard tile, per-agent tab, per-automation view, per-user members strip — admin-gated (§8).
...
- **Device-CLI automation cost capture** (§5.4) — follow-up; flag as uncaptured in v1.
...
| 6 | Always a `runs` row at completion with the dims? | **Verify before PR-2** — `org_id` always; `automation_id` first-class for automation runs; `agent_id`/`user_id`/`conversation_id` live in `action_input` JSONB → denormalize onto `run_usage`. Confirm chat/agent runs always create a `runs` row. | Ledger correctness |
...
| 8 | Device-CLI automation runs (Owletto) cost | **Out of scope v1** — no gateway-side `session.jsonl`, report no cost today; flag as uncaptured; later via claude/codex CLI cost output. Cloud automation path unaffected. | No |
...
- One **per-org "Spend (14d)" tile** off `run_usage` via the existing `metric_series` endpoint. Ship it, see if anyone looks. Only then build per-agent/automation/user breakdowns and caps.
...
- Add index `(organization_id, user_id, occurred_at)` — per-user is a wanted breakdown; only org/agent/automation indexes exist.
