...
/**
 * Metadata keys stamped by behavior promotion (`promote-keyed-entities.ts`):
 ...
 */
const BEHAVIOR_PROVENANCE_KEYS = ['behavior_id', 'stable_key', 'window_id'];

function withoutBehaviorProvenanceKeys(
  metadata: Record<string, unknown>
): Record<string, unknown> {
  if (!BEHAVIOR_PROVENANCE_KEYS.some((key) => key in metadata)) return metadata;
  return Object.fromEntries(
    Object.entries(metadata).filter(([key]) => !BEHAVIOR_PROVENANCE_KEYS.includes(key))
  );
}
...
export async function validateEntityMetadata(
  ...
  // they are stamped by behavior promotion outside this validator and would
  // otherwise fail every round-trip under additionalProperties: false.
  const ajv = getAjv();
  const validate = ajv.compile(schema);
  const candidate = withoutBehaviorProvenanceKeys(metadata);
