 ...
 *     the existing `behavior_versions` row instead of duplicating it.
 *   - Editing one assignment via `create_version` cascades to every behavior
 ...
 *     `behavior_versions` ownership to a surviving sibling so the cascade
...
import { parseBehaviorRunPayload } from '../../../behaviors/automation';
...
async function seedRootBehavior(workspace: TestWorkspace, suffix: string) {
  ...
  return { behaviorId: Number(behavior.behavior_id), entityId: entity.id };
}
...
describe('behavior group edit contract', () => {
  beforeEach(async () => {
  ...
  it('create_from_version reuses the source version row instead of duplicating', async () => {
    ...
    const { behaviorId: rootId } = await seedRootBehavior(workspace, 'reuse');

    const [rootRow] = await sql`
      SELECT current_version_id, behavior_group_id FROM behaviors WHERE id = ${rootId}
    `;
    const rootVersionId = Number(rootRow.current_version_id);
    const groupId = Number(rootRow.behavior_group_id);
    ...
    // All three behaviors should point at the SAME version row.
    const rows = await sql`
      SELECT id, current_version_id, behavior_group_id
      FROM behaviors WHERE id IN (${rootId}, ${sibling1Id}, ${sibling2Id})
      ORDER BY id
    `;
    expect(rows).toHaveLength(3);
    for (const row of rows) {
      expect(Number(row.current_version_id)).toBe(rootVersionId);
      expect(Number(row.behavior_group_id)).toBe(groupId);
    }

    // behavior_versions row count for this group is exactly 1, not 3.
    const versionCount = await sql`
      SELECT COUNT(*)::int as n FROM behavior_versions WHERE behavior_id = ${groupId}
    `;
  ...
  it('create_from_version copies the reaction script AND its input schema onto each new assignment', async () => {
    ...
    const { behaviorId: rootId } = await seedRootBehavior(workspace, 'script-copy');
    ...
    const [rootRow] = await sql`
      SELECT current_version_id FROM behaviors WHERE id = ${rootId}
    `;
    const rootVersionId = Number(rootRow.current_version_id);
    ...
    const [siblingRow] = await sql`
      ...
      FROM behaviors WHERE id = ${siblingId}
    `;
  ...
  it('set_reaction_script extracts the exported input schema to reaction_input_schema', async () => {
    ...
    const { behaviorId: rootId } = await seedRootBehavior(workspace, 'react-input');
    ...
    const [row] = await sql`
      SELECT reaction_input_schema FROM behaviors WHERE id = ${rootId}
    `;
  ...
  it('create_version cascades current_version_id and name across the whole group', async () => {
    ...
    const { behaviorId: rootId } = await seedRootBehavior(workspace, 'cascade');
    ...
    const [versionRow] = await sql`
      SELECT behavior_id, prompt FROM behavior_versions WHERE id = ${newVersionId}
    `;
    expect(Number(versionRow.behavior_id)).toBe(rootId);
    expect(versionRow.prompt).toBe('Cascaded prompt v2.');
  });

  it('set_reaction_script cascades to every behavior in the group', async () => {
    const sql = getTestDb();
    const workspace = await TestWorkspace.create({ name: 'Group Script Org' });
    const { behaviorId: rootId } = await seedRootBehavior(workspace, 'script-cascade');
    const [rootBefore] = await sql`
      SELECT current_version_id FROM behaviors WHERE id = ${rootId}
    `;
    const siblingEntity = await createTestEntity({
    ...
    let rows = await sql`
      SELECT id, reaction_script FROM behaviors WHERE id IN (${rootId}, ${siblingId}) ORDER BY id
    `;
  ...
  it('createBehaviorRun snapshots current_version_id; mid-run group edit does not change the run', async () => {
    const sql = getTestDb();
    const workspace = await TestWorkspace.create({ name: 'Run Snapshot Org' });
    const { behaviorId: rootId } = await seedRootBehavior(workspace, 'snapshot');
    ...
    // to v2 on the behaviors row, but the run's payload still holds v1.
    await manageBehaviors(
      {
        action: 'create_version',
        behavior_id: String(rootId),
    ...
    // The behavior itself has moved on — confirms the snapshot diverges.
    const [behaviorAfter] = await sql`
      SELECT current_version_id FROM behaviors WHERE id = ${rootId}
    `;
    expect(Number(behaviorAfter.current_version_id)).not.toBe(snapshotVersionId);
  });

  it('parseBehaviorRunPayload returns the snapshot version_id', async () => {
    const sql = getTestDb();
    const workspace = await TestWorkspace.create({ name: 'Payload Parse Org' });
    const { behaviorId: rootId } = await seedRootBehavior(workspace, 'parse');
    ...
    const parsed = parseBehaviorRunPayload(run.approved_input);
    expect(parsed).not.toBeNull();
    expect(parsed!.version_id).not.toBeNull();
    expect(Number.isFinite(parsed!.version_id as number)).toBe(true);
  });

  it('parseBehaviorRunPayload tolerates legacy runs missing version_id', () => {
    const legacyPayload = {
      behavior_id: 1,
      agent_id: 'a',
    ...
    const parsed = parseBehaviorRunPayload(legacyPayload);
    expect(parsed).not.toBeNull();
    expect(parsed!.version_id).toBeNull();
  });

  it('complete_window scopes the run lookup by behavior_id', async () => {
    const sql = getTestDb();
    const workspace = await TestWorkspace.create({ name: 'Run Scope Org' });
    const { behaviorId: aId } = await seedRootBehavior(workspace, 'scope-a');
    const { behaviorId: bId } = await seedRootBehavior(workspace, 'scope-b');

    // Create a run for behavior A. The run's snapshot version is A's current.
    const aRun = await createBehaviorRun({
      organizationId: workspace.org.id,
      behaviorId: aId,
      agentId: 'a-agent',
      windowStart: new Date(Date.now() - 60 * 60 * 1000).toISOString(),
    ...
    // snapshot when scoped by behavior_id = B.
    const [scopedToB] = await sql`
      SELECT (approved_input->>'version_id')::bigint AS version_id
      FROM runs WHERE id = ${aRun.runId} AND behavior_id = ${bId}
      LIMIT 1
    `;
    expect(scopedToB).toBeUndefined();
    const [scopedToA] = await sql`
      SELECT (approved_input->>'version_id')::bigint AS version_id
      FROM runs WHERE id = ${aRun.runId} AND behavior_id = ${aId}
      LIMIT 1
    `;
  ...
  it('serializes concurrent create_version calls on the same group', async () => {
    ...
    const { behaviorId: rootId } = await seedRootBehavior(workspace, 'concurrent');
    ...
    // neither collides on (behavior_id, version) unique index.
    ...
    const versionRows = await sql`
      SELECT version FROM behavior_versions WHERE behavior_id = ${rootId} ORDER BY version
    `;
