	...
	return {
		...
		queue: (ctx) =>
			proposeEntityCreate(ctx, {
				...
				proposal: args.proposal,
				behavior_id: args.behaviorId ?? null,
...
async function evaluate(
	...
	| null
> {
	// A behavior acting under its owning agent folds the AGENT'S rows in too, so
	// the agent envelope binds while a pre-existing behavior-specific restriction
	// can only tighten (the agent envelope never loosens it away).
	const ownerAgentId = req.ownerAgentId ?? null;
	// A behavior whose owning agent couldn't be resolved fails closed downstream.
		...
		deferred: {
			...
			queue: (ctx) =>
				proposeEntityDelete(ctx, {
					...
					behavior_id: req.behaviorId ?? null,
