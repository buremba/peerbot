 ...
 *
 * A behavior's `agent_id` IS its policy principal — every write the behavior performs
 * folds that agent's envelope. So a NON-HUMAN caller must not be able to set/change
 * a behavior's agent_id to a DIFFERENT agent: agent A (with a deny envelope) could
 * otherwise mint/reassign a behavior owned by looser agent B and route its writes
...
async function createBehaviorAs(
	ctx: AuthContext,
	agentIdForBehavior: string,
	slug: string
) {
	return executeTool(
		"manage_behaviors",
...
describe("manage_behaviors owner-escalation guard", () => {
	...
	it("blocks agent A from creating a behavior owned by agent B (no principal reassignment)", async () => {
		await expect(
			createBehaviorAs(
	...
	it("a HUMAN may assign a behavior to any agent (ungoverned)", async () => {
	...
	it("blocks agent A from CLONING agent B's behavior via create_from_version (inherited owner)", async () => {
		// A human seeds a behavior owned by B; its v1 version_id is the clone source.
		...
		const bBehavior = (await workspace.owner.behaviors.create({
			slug: "b-owned-source",
			name: "b-owned-source",
			prompt: "Track things.",
			agent_id: agentB,
      ...
      SELECT id FROM behavior_versions WHERE behavior_id = ${Number(bBehavior.behavior_id)} ORDER BY id ASC LIMIT 1
    `;
	...
	it("blocks agent A from EDITING agent B's behavior (preserved owner, no agent_id)", async () => {
		const bBehavior = (await workspace.owner.behaviors.create({
			slug: "b-owned-edit",
			name: "b-owned-edit",
			prompt: "Track things.",
		...
		await expect(
			executeTool(
				...
				{
					action: "update",
					behavior_id: bBehavior.behavior_id,
	...
	it("set_reaction_script on A's own behavior is blocked when its GROUP also contains a B-owned assignment (codex-12)", async () => {
		// A owns behavior wA; a human adds a SECOND assignment owned by B into wA's group
		// (same behavior_group_id). set_reaction_script writes group-wide → it would
		// rewrite B's reaction code too. A editing "its own" behavior must be blocked.
		...
		// a direct UPDATE — a raw behavior INSERT trips unrelated sequence collisions.
		...
		const [grp] = await getTestDb()<{ behavior_group_id: number }>`
      SELECT behavior_group_id FROM behaviors WHERE id = ${Number(wA.behavior_id)} LIMIT 1
    `;
		await getTestDb()`
      UPDATE behaviors SET behavior_group_id = ${Number(grp.behavior_group_id)}
