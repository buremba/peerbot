 ...
 *      test provider whose tokenUrl points at a LOCAL fake provider server that
 *      returns canned tokens.
 *   2. The broker's `POST /broker/oauth/exchange` is PAT-gated. A valid PAT for
 *      the broker org → the broker resolves ITS managed app, uses the secret,
 *      and returns ONLY the user's tokens. No/invalid PAT → 401.
 *   3. End-to-end: a LOCAL connect token whose oauth_app profile is a
...
// Fake OAuth provider: the tokenUrl the broker's managed app points at.
...
/** Seed a broker org with a managed oauth_app (fake creds) for `demo`. */
...
describe("SPIKE: broker-delegated oauth_app — exchange handshake", () => {
	...
	it("PAT-gated exchange resolves the broker managed app and returns user tokens", async () => {
	...
	it("authorize-url builds the provider consent URL with the broker client_id", async () => {
		const { pat } = await seedBrokerOrg();
		const app = buildBrokerApp();
		const res = await app.fetch(
			new Request("http://broker.local/broker/oauth/authorize-url", {
				...
				body: JSON.stringify({
					...
					scopes: ["read"],
					state: "state-xyz",
					authorization_url: "https://demo.example/authorize",
				}),
			}),
			TEST_ENV,
		);
...
describe("SPIKE: broker-delegated oauth_app — end-to-end local delegation", () => {
	...
	it("local callback delegates code exchange to the broker and stores broker tokens", async () => {
		// Broker side: managed app + PAT (broker.url = the in-process broker server).
		const { pat } = await seedBrokerOrg();
    ...
    `) as unknown as Array<{ auth_data: Record<string, unknown> }>;
		...
		// Connect token (oauth) bound to the local broker-ref app profile. created_by
		// set so the callback skips session lookup.
		...
		// POSTs the broker /exchange (which uses the broker's secret) and stores
		// the broker-returned tokens.
		const app = new Hono<{ Bindings: Env }>();
