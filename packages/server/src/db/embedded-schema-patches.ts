...
export const EMBEDDED_SCHEMA_PATCHES: EmbeddedSchemaPatch[] = [
  ...
  },
  {
    ...
    //
    // Sequenced after `watcher-schema-additions` (#799) when that patch
    // lands; the only watcher-side change here is the new column, which
    // composes cleanly with whatever columns #799 adds.
    id: 'goals-primitive',
    apply: async (sql) => {
      await sql.unsafe(`
