...
- **Behavior is the only live vocabulary.** The retired term `watcher` may remain only in immutable migration history and naming-guard negative fixtures. It must not appear in live code, identifiers, database schema, MCP/ClientSDK contracts, operational scripts, or current docs — `make pre-pr` fails on it.
