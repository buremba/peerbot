...
/**
 * Short stable id for an inline judge — first 8 chars of sha256(policy).
 * Used in the generated guardrail name (`inline:<stage>:<hash8>`) so the
 * operator's `guardrails_disabled` list can target a specific inline judge.
 */
export function inlineJudgeHash(policy: string): string {
  return createHash("sha256").update(policy).digest("hex").slice(0, 8);
}
...
export function createJudgeGuardrail<S extends GuardrailStage>(
  ...
  const name = options.name ?? `inline:${stage}:${inlineJudgeHash(policy)}`;
  const toolFilter =
