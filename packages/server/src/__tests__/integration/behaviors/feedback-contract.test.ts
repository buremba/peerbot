/**
 * Compact behavior feedback contract.
 *
 * High-value coverage retained from the deleted feedback suite: the feedback
 * API is the durable human-correction path for behavior outputs, so it must
...
async function seedBehavior(workspace: TestWorkspace, suffix: string) {
  ...
  const behavior = (await workspace.owner.behaviors.create({
    entity_id: entity.id,
    slug: `feedback-behavior-${suffix}`,
    name: `Feedback Behavior ${suffix}`,
  ...
  return { behaviorId: behavior.behavior_id, windowId };
}
...
describe('behavior feedback contract', () => {
  ...
  it('stores set/remove/add field corrections from one batch as separate correction events', async () => {
    ...
    const rows = await getTestDb()`
      ...
      FROM events
      WHERE semantic_type = 'correction' AND (metadata->>'behavior_id')::bigint = ${Number(behaviorId)}
  ...
  it('feedback ids are the correction event ids; historical wwff_ origin_ids still parse', async () => {
    // Post-3b (behavior_window_field_feedback_id_seq dropped): a new correction's
    ...
    await sql`
      ...
      VALUES (${workspace.org.id}, 'correction', '{}'::bigint[], 'wwff_424242',
        ${sql.json({ window_id: Number(windowId), behavior_id: Number(behaviorId), field_path: 'legacy.field', mutation: 'set', corrected_value: 'old', note: null })},
        NOW(), NOW())
    `;
  ...
  it('rejects malformed corrections and cross-org behavior/window ids', async () => {
    await expect(
      manageBehaviors(
        {
          action: 'submit_feedback',
    ...
    const foreign = await seedBehavior(other, 'foreign');
    await expect(
      manageBehaviors(
        {
          action: 'submit_feedback',
          behavior_id: foreign.behaviorId,
   ...
   */
  // Canvas-on-events: seedBehavior already creates the canvas chain ROOT (its id
  ...
  it('materializes a superseding canvas_state with the correction applied AND still writes advisory events', async () => {
    const seeded = await seedBehavior(workspace, `materialize-${Date.now()}`);
    const rootId = seeded.windowId;

    const result = (await manageBehaviors(
      {
        action: 'submit_feedback',
        behavior_id: seeded.behaviorId,
    ...
    const advisory = await getTestDb()`
      ...
      WHERE semantic_type = 'correction'
        AND (metadata->>'behavior_id')::bigint = ${Number(seeded.behaviorId)}
    ...
    const head = await getTestDb()`
      ...
      WHERE e.semantic_type = 'canvas_state'
        AND (e.metadata->>'behavior_id')::bigint = ${Number(seeded.behaviorId)}
  ...
  it('concurrent supersede of the same head loses with 409', async () => {
    const seeded = await seedBehavior(workspace, `concurrent-${Date.now()}`);
    const rootId = seeded.windowId;
      ...
      await insertEvent(
        {
          ...
          metadata: {
            behavior_id: Number(seeded.behaviorId),
  ...
  it('a prototype-polluting field_path is inert (advisory recorded, payload and prototypes untouched)', async () => {
    const seeded = await seedBehavior(workspace, `pollute-${Date.now()}`);
    // seedBehavior already created the canvas root (payload { problems: [...] }).
    ...
    // The seedBehavior root payload is unchanged; the forbidden paths were no-ops.
  ...
  it('rejects a window_id that is not a live canvas chain root', async () => {
    ...
    const seeded = await seedBehavior(workspace, `nochain-${Date.now()}`);
    await expect(
      manageBehaviors(
        {
          action: 'submit_feedback',
