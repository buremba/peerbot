...
describe("resolvePinnedSelection", () => {
  ...
  it("never materializes a conversations row for an automation run (honors the exclusion)", async () => {
    // Automation runs are ephemeral and have NO conversations row (message-consumer
    ...
    // the sidebar and break the automation no-row contract).
    const org = await createTestOrganization({ name: "Pin Automation Org" });
    const agent = await createTestAgent({ organizationId: org.id });
    const sandbox = await createSandbox(org.id, {
    ...
    const automationConvId = `${agent.agentId}_automation_1_run_42`;
    const sel = await resolvePinnedSelection({
      organizationId: org.id,
      agentId: agent.agentId,
      platform: PLATFORM,
      conversationId: automationConvId,
    });

    // Live selection returned (the agent's current provider) …
    expect(sel.runtimeProviderId).toBe("vercel");
    // … but NO row was created for the automation conversation id.
    const sql = getTestDb();
    const rows = (await sql`
      SELECT 1 FROM public.conversations
      WHERE organization_id = ${org.id} AND agent_id = ${agent.agentId}
        AND conversation_id = ${automationConvId}
    `) as unknown[];
