...
const base = { organization_id: 'orgB', device_owner: null, automation_device_owner: null };
...
describe('runInWorkerScope', () => {
  ...
  it('in scope when the worker owns the run\'s pinned automation device (cross-org)', () => {
    expect(
      runInWorkerScope(
        { ...base, automation_device_owner: 'u1' },
        { workerUserId: 'u1', orgIds: ['orgA'] }
      )
    ).toBe(true);
  });
  ...
  it('forbidden when org is out of scope and the worker owns neither pinned device', () => {
    expect(
      runInWorkerScope(
        { ...base, device_owner: 'someone-else', automation_device_owner: 'someone-else' },
        { workerUserId: 'u1', orgIds: ['orgA'] }
      )
    ).toBe(false);
  });
  ...
  it('forbidden when there is no worker user and the org is out of scope', () => {
    expect(
      runInWorkerScope({ ...base, automation_device_owner: 'u1' }, { workerUserId: null, orgIds: [] })
    ).toBe(false);
  });
});
