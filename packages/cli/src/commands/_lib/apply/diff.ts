...
function diffWatcher(
	...
	// name/description are scalar row fields (manage_behaviors update accepts
	// them). Only compare when declared so an omitted name inherits the
	// server default without perpetual churn.
...
export function computeDiff(
	...
	if (only === undefined) {
		...
		for (const provider of desiredProviders) {
			rows.push(
				diffInferenceProvider(
					provider,
					remoteProviderBySlug.get(provider.slug),
				),
			);
		}
