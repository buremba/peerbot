...
- **Behavior is the only live vocabulary.** The retired term `watcher` must not appear in live code, identifiers, database schema, MCP/ClientSDK contracts, operational scripts, or current docs — `make pre-pr` fails on it. It may remain only where the guard exempts it: `db/migrations/` (immutable history), tests that replay or assert a pre-cutover migration, the naming guard and its negative fixtures, `CHANGELOG.md`, and these `AGENTS.md` files. Adding an exemption means editing `scripts/check-exposed-surface-naming.ts` — keep this list in step with it.
