...
const BASE_COLUMNS_SQL = `f.id, f.entity_ids, f.connection_id, f.feed_id, f.feed_key, f.behavior_id, fd.display_name as feed_name, COALESCE(wv.name, 'watcher-' || f.behavior_id) as behavior_name, f.payload_text, f.title, f.author_name, f.source_url, f.occurred_at, f.semantic_type,
