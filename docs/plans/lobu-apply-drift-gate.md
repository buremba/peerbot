...
2. **Un-declared remote state is never clobbered or auto-deleted.** With a recorded baseline, a remote-only change or definition blocks the apply and is reported with an **adopt** snippet or a **named delete** (`--confirm-deletes`). Before attribution exists, remote-only state is informational unless prune requests a deletion, which blocks for lack of ownership proof.
...
Full and `--only memory` applies record the memory/Behavior baseline. `--only agents` preserves the prior baseline and leaves it absent when none exists. Bootstrap populates `attribution` and an **empty `owned` set**.
