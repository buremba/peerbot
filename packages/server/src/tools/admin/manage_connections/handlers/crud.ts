...
/**
 * The channels a connection reaches — its `agent_channel_bindings`. A binding
 * links a (platform, channel, team) to an agent; "reach" is the set of those
 * for this connection. Bindings carry a `connection_id` (Stage 1), but it isn't
 * populated on the create path yet, so we also fall back to matching by
 * (platform, team) — a connection's team is its `external_tenant_id`. Each row
 * is surfaced in the UI as "via <connection>".
 */
export async function handleListReach(
  args: Extract<ConnectionsArgs, { action: 'list_reach' }>,
  ctx: ToolContext
): Promise<ManageConnectionsResult> {
  const sql = getDb();
  const { organizationId } = ctx;

  // Resolve + visibility-gate the connection exactly like handleGet.
  let connQuery = sql`
  ...
  `;
  if (!ctx.userId) {
    connQuery = sql`${connQuery} AND visibility = 'org'`;
  } else {
    const userRole = await getWorkspaceRole(sql, organizationId, ctx.userId);
    if (userRole !== 'owner' && userRole !== 'admin') {
      connQuery = sql`${connQuery} AND (visibility = 'org' OR created_by = ${ctx.userId})`;
    }
  }
  const connRows = await connQuery;
  if (connRows.length === 0) return { error: 'Connection not found' };
  const conn = connRows[0] as {
    connector_key: string;
    external_tenant_id: string | null;
  };

  // Fallback match by (platform, team) for bindings not yet linked by id.
  const teamClause = conn.external_tenant_id
    ? sql`AND team_id = ${conn.external_tenant_id}`
  ...
  const rows = await sql`
    ...
    WHERE organization_id = ${organizationId}
      AND (
        ...
        OR (connection_id IS NULL AND platform = ${conn.connector_key} ${teamClause})
      )
    ORDER BY channel_id
  `;
