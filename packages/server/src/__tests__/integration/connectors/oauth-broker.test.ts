...
beforeAll(async () => {
  ...
  attackerServer = await new Promise((resolve) => {
    const s = serve({ fetch: attackerApp.fetch, hostname: '127.0.0.1', port: 0 }, (info) => {
...
afterAll(async () => {
  await new Promise<void>((done) => (providerServer ? providerServer.close(() => done()) : done()));
  await new Promise<void>((done) => (attackerServer ? attackerServer.close(() => done()) : done()));
  await new Promise<void>((done) => (brokerServer ? brokerServer.close(() => done()) : done()));
});
...
async function seedBrokerOrg() {
  ...
  await createAuthProfile({
    ...
    provider: 'demo',
    authData: { DEMO_CLIENT_ID: 'broker-cid', DEMO_CLIENT_SECRET: 'broker-secret' },
  });
...
describe('SPIKE: broker-delegated oauth_app — exchange handshake', () => {
  ...
  it('authorize-url builds the provider consent URL with the broker client_id + server-resolved endpoint', async () => {
    ...
    expect(`${url.origin}${url.pathname}`).toBe('https://demo.example/authorize');
...
describe('SPIKE: broker-delegated oauth_app — end-to-end local delegation', () => {
  ...
  it('local callback delegates code exchange to the broker and stores broker tokens', async () => {
    ...
    await createTestConnectorDefinition({
      ...
      auth_schema: { methods: [{ type: 'oauth', provider: 'demo', requiredScopes: ['read'] }] },
      feeds_schema: { items: {} },
    });
    ...
    `) as unknown as Array<{ accessToken: string; refreshToken: string; providerId: string }>;
