/**
 * Timezone-aware schedule evaluation for behaviors and feeds.
 *
 ...
 * pre-existing server-time behavior. Asia/Taipei (UTC+8, no DST) makes the
...
describe('behaviors/feeds timezone-aware schedules', () => {
  ...
  beforeAll(async () => {
    ...
    await createTestAgent({
      ...
      agentId: 'tz-behavior-agent',
      name: 'TZ Behavior Agent',
    });
  ...
  it('Behavior create stores trigger timezone and anchors next_run_at to the zone wall-clock', async () => {
    const created = (await workspace.owner.behaviors.create({
      ...
      agent_id: 'tz-behavior-agent',
    })) as { behavior_id: string };
    const behaviorId = Number(created.behavior_id);

    const sql = getTestDb();
    const [row] = await sql`
      SELECT timezone, next_run_at FROM behaviors WHERE id = ${behaviorId}
    `;
    ...
    await sql`
      UPDATE behaviors SET next_run_at = NOW() - INTERVAL '1 minute' WHERE id = ${behaviorId}
    `;
    await advanceBehaviorSchedule(sql as unknown as DbClient, behaviorId);
  ...
  it('Behavior update re-anchors the pending firing when trigger timezone changes', async () => {
    const created = (await workspace.owner.behaviors.create({
      entity_id: entityId,
      slug: 'tz-behavior-update',
      name: 'TZ Behavior Update',
      prompt: 'Summarize {{entities}}.',
    ...
    const sql = getTestDb();
    const [before] = await sql`SELECT next_run_at FROM behaviors WHERE id = ${behaviorId}`;
