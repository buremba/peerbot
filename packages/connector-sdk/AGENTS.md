...
- `egress-policy.ts` (`@lobu/connector-sdk/egress-policy`) is the ONE egress pattern grammar and decision order for the whole monorepo: the gateway proxy, the grant and policy stores, the remote runtime providers and the connector isolate lane all match hosts through it. It is pure (no `node:`, no `@lobu/core`) so the same code runs on the Node host, inside an isolate, and on workerd. Change the grammar here or nowhere; configured patterns are normalized once by `@lobu/core`'s `normalizeDomainPattern`.
