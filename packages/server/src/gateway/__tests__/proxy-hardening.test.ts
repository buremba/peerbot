...
describe("egress denylist — trailing-dot FQDN canonicalization", () => {
  ...
  test("blocks a denylisted host written with a trailing dot", async () => {
    const plain = await __testOnly.checkDomainAccess("pastebin.com", undefined, undefined);
    expect(plain.allowed).toBe(false);

    // The bug: this returned allowed:true before the canonicalization fix.
    const dotted = await __testOnly.checkDomainAccess("pastebin.com.", undefined, undefined);
    expect(dotted.allowed).toBe(false);

    // And multiple trailing dots.
    const multiDot = await __testOnly.checkDomainAccess("pastebin.com..", undefined, undefined);
    expect(multiDot.allowed).toBe(false);
  });

  test("still allows a non-denylisted host (trailing dot or not) in unrestricted mode", async () => {
    expect((await __testOnly.checkDomainAccess("example.com", undefined, undefined)).allowed).toBe(true);
    expect((await __testOnly.checkDomainAccess("example.com.", undefined, undefined)).allowed).toBe(true);
  });
});
