/**
 * Streaming (chat-channel) feed as a behavior @feed source — the membership-gated
 ...
 *
 *   1. On a NON-enforced connection, a headless behavior run (null principal)
...
import { normalizeBehaviorSources } from "../../../behaviors/source-refs";
...
describe("streaming feed as a behavior @feed source", () => {
  ...
  beforeEach(async () => {
    ...
    await sql`
      ...
      WHERE organization_id = ${orgId}
        AND (metadata->>'source' = 'behavior_promotion' OR entity_type_id IN (
   ...
   *  reader — exactly the behavior read path (normalize → executeDataSources). */
  async function readAsBehaviorSource(
    ...
    const sql = getTestDb();
    const normalized = await normalizeBehaviorSources(sql as unknown as DbClient, orgId, [
  ...
  it("a streaming @feed compiles to kind 'channel' (prompt context, not event-signed)", async () => {
    ...
    // complete_window would insert them into behavior_window_events.event_id (FK
  ...
  it("a headless behavior reads a NON-enforced channel's transcript", async () => {
    const conn = await makeChatConnection();
    await ensureStreamingChannelFeed({
      connectionId: conn.id,
      organizationId: orgId,
    ...
    const rows = (await readAsBehaviorSource(null)) as Array<{ text: string }>;
  ...
  it("a headless behavior reads NOTHING from an ACL-enforced channel (no leak)", async () => {
    const conn = await makeChatConnection();
    await ensureStreamingChannelFeed({
      connectionId: conn.id,
      organizationId: orgId,
    ...
    const rows = await readAsBehaviorSource(null);
  ...
  it("a channel MEMBER reads the enforced channel (gate is membership, not denial)", async () => {
    ...
    const rows = (await readAsBehaviorSource(readerUserId)) as Array<{ text: string }>;
    expect(rows.length).toBe(2);
    expect(rows.map((r) => r.text).sort()).toEqual([
      "channel secret one",
      "channel secret two",
  ...
  it("does not expose a PRIVATE connection's channel to a headless behavior (connection visibility)", async () => {
    ...
    // passthrough) would leak it to a headless behavior; connection visibility
    // must also apply.
    chatConnectionSeq += 1;
    ...
    expect((await readAsBehaviorSource({ userId: null })).length).toBe(0);
    // The creator can read it.
    expect(
      (await readAsBehaviorSource({ userId: workspace.users.owner.id })).length
    ).toBe(2);
  });

  it("a channel @feed source respects the behavior window bounds", async () => {
    const conn = await makeChatConnection();
    await ensureStreamingChannelFeed({
      connectionId: conn.id,
      organizationId: orgId,
    ...
    const rows = (await readAsBehaviorSource({
      userId: null,
      windowStart,
      windowEnd,
