...
Full and `--only memory` applies record the memory/Behavior baseline. `--only agents` preserves the prior baseline and leaves it absent when none exists.
...
- ~~`--bootstrap`~~ **dropped, not deferred.** It solved a problem that does not exist: an org with no recorded baseline gets one from its next successful full or memory-scoped apply, including an all-noop run. A first `--only agents` apply intentionally leaves the memory/Behavior baseline absent because it does not execute that family. Verified end-to-end — an org forced back to an attribution-less manifest recorded `attribution` + `owned` after a single `0 create, 0 update, 1 noop` apply. Snapshotting remote into `attribution` with an empty `owned` set would produce the same classifications the unrecorded path already produces, so the flag would add a command surface and buy nothing. If a bootstrap-shaped need resurfaces, the useful version is the opposite direction — *adopt* remote definitions into `lobu.config.ts` so they become declared — which is the adopt-snippet codegen below, not a baseline snapshot.
...
- **Naming**: user/agent-facing surface says **Behavior**, never `watcher`. The deferred `--confirm-deletes` flow covers both delete (definition) and revert (field); its token decode names each item's action.
