...
# (event_id, classifier_id, source, COALESCE(automation_id,0)).
#
# WHY: today uniqueness is per (event_id, classifier_VERSION_id, source, automation). Collapsing the
# key onto the stable classifier_id can collide IFF a single (event, classifier, source, automation)
...
while :; do
  ...
  N=$("${PSQL[@]}" "
    WITH dupes AS (
      SELECT event_id, classifier_id, source, COALESCE(automation_id, 0) AS w, MAX(id) AS keep_id
      FROM event_classifications
      WHERE classifier_id IS NOT NULL
      GROUP BY event_id, classifier_id, source, COALESCE(automation_id, 0)
      HAVING count(*) > 1
      LIMIT ${BATCH}
    ), del AS (
      ...
      WHERE ec.event_id = d.event_id
        ...
        AND COALESCE(ec.automation_id, 0) = d.w
        AND ec.id <> d.keep_id
      RETURNING 1
