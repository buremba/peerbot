...
async function seedRows(options?: { duplicateIdentifier?: boolean }) {
  ...
  `;
	...
	const identifiers = options?.duplicateIdentifier
		? ["CARI-001", "CARI-001"]
		: ["CARI-001", "CARI-002"];
	const scopes = options?.duplicateIdentifier
		? ["legacy-a", "legacy-b"]
		: [null, null];
...
describe("connector identity scope lifecycle", () => {
		...
		expect(metricAliases).toEqual([
			{
				aliases: [],
				scoped_aliases: [
					{ namespace, identifier: "CARI-001", scopeKey: "tenant-a" },
				],
			},
			{
				aliases: [],
				scoped_aliases: [
					{ namespace, identifier: "CARI-002", scopeKey: "tenant-b" },
				],
			},
		]);
    ...
    `;
		...
		await expect(
			...
			/erp_customer.*2 live identity rows.*Old shape.*organization.*New shape.*tenant/i,
		);
	...
	it("requires every connector sharing a namespace to converge before re-key", async () => {
		...
		await expect(
			...
			/scope-lifecycle-secondary.*every connector sharing the namespace/i,
		);
    ...
    `;
		expect(registryRows).toEqual([
			...
			{
				connector_key: secondConnectorKey,
				scope: "tenant",
				pending_scope: null,
			},
		]);
	...
	it("backfills pre-registry peer connectors and rejects a conflicting new connector", async () => {
		const org = await createTestOrganization({
			name: "Shared namespace registry org",
		});
    ...
    `;
		expect(rows).toEqual([
			{
				connector_key: firstConnector,
				scope: "organization",
				scope_key_path: null,
			},
			{
				connector_key: secondConnector,
				scope: "organization",
				scope_key_path: null,
			},
		]);
	...
	it("keeps registry and active definition aligned across concurrent installs", async () => {
		const org = await createTestOrganization({
			name: "Concurrent scope install org",
		});
		await install(org.id, metadata("1.0.0", {}));
		await Promise.all([
    ...
    `;
		...
		expect(rows[0]?.registry_scope).toBe(
			rows[0]?.active_scope ?? "organization",
		);
	});
