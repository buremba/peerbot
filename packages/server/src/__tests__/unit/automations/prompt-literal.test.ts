...
describe("automation prompt text is literal", () => {
	...
	it("window dispatch embeds the frozen prompt verbatim, braces included", () => {
		const message = buildDispatchMessage({
			...
			automationInstructions: prompt,
		});

		expect(message).toContain("Automation instructions:");
	...
	it("event dispatch embeds the frozen prompt verbatim, braces included", () => {
		const message = buildDispatchMessage({
			...
			runId: 647147,
			agentId: "personal-agent",
			sessionAgentId: "personal-agent_automation_13_run_647147",
			payload: {
				automation_id: 13,
				agent_id: "personal-agent",
				window_start: "2026-07-15T00:00:00.000Z",
	...
	it("window dispatch falls back to a generic instruction when the version has no prompt", () => {
		const message = buildDispatchMessage({
			...
			runId: 647148,
			agentId: "personal-agent",
			sessionAgentId: "personal-agent_automation_13_run_647148",
			payload: {
				automation_id: 13,
				agent_id: "personal-agent",
				window_start: "2026-07-15T00:00:00.000Z",
