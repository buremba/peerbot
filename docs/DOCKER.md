...
**Agent runtime**: Lobu's worker runs its native Pi-based agent loop per task. The behavior table can still select a CLI agent kind where a behavior explicitly drives one. Runtime choice is independent from the LLM provider serving the agent.
