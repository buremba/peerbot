/**
 * One-off backfill: re-stamp `events.origin_id` on Behavior event-output rows
 * that were written with a POSITIONAL origin_id.
 *
 * Context: `persistBehaviorEventOutput` used to name every row
 * `behavior_<id>_<output>_canvas_<revision>_<arrayIndex>`. An hourly Behavior's
 * runs all append to the same Canvas revision, so item N of every run got the
 * same origin_id while being a different source item. Prod Behavior 71 put 8
 * unrelated posts under `behavior_71_signals_canvas_4819569_4`. The rows are all
 ...
 * produced, by calling `behaviorOutputOriginId` rather than reimplementing the
 ...
 * (`behavior:<id>:output:<name>:key:<sourceKey>`) are touched. A row keyed
...
import { behaviorOutputOriginId } from '../utils/persist-behavior-event-output';

export interface BackfillBehaviorOutputOriginIdOptions {
  db: DbClient;
...
export interface BackfillBehaviorOutputOriginIdReport {
...
function sourceKeyFrom(row: CandidateRow): string | null {
  if (!row.idempotency_key || !row.behavior_id || !row.behavior_output) return null;
  const prefix = `behavior:${row.behavior_id}:output:${row.behavior_output}:key:`;
...
/**
 * Re-stamp positional Behavior event-output origin_ids. Walks by ascending
 * event id so an interrupted run resumes cleanly.
 */
export async function backfillBehaviorOutputOriginId(
  opts: BackfillBehaviorOutputOriginIdOptions
): Promise<BackfillBehaviorOutputOriginIdReport> {
  const sql = opts.db;
  ...
  for (;;) {
    const candidates = (await sql`
      SELECT id,
             origin_id,
             metadata->>'behavior_id' AS behavior_id,
             metadata->>'behavior_output' AS behavior_output,
             metadata->>'_lobu_idempotency_key' AS idempotency_key
      FROM events
      WHERE id > ${cursor}
        AND origin_id IS NOT NULL
        AND metadata ? 'behavior_output'
    ...
    for (const row of candidates) {
      ...
      const derived = behaviorOutputOriginId(
        Number(row.behavior_id),
        String(row.behavior_output),
        sourceKey
      );
