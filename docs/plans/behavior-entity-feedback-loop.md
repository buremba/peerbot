# Behavior ↔ Entity sync + human–AI feedback loop

> **Status (2026-07-15):** **Shipped** — `entity_field_controls` migration + `mergeEntityFields` feedback path live.

## Goal
Behaviors sync extracted values **into entities** (the entity is the source of truth), and a
human can correct an entity field; the behavior then **respects** that correction and the
...
- **Ownership** → `entities.field_controls jsonb` **column** (NOT a side table): a key present means
  ...
  No `mode` enum — human-owned simply means *behavior proposes via approval, never writes directly*.
- **One merge primitive** → `mergeEntityFields({source})` shared by human edits (`updateEntity`) and
  behavior promotion. `human` writes every changed field and marks it owned; `behavior` writes only
...
2. **Promotion:** `promoteKeyedEntities` calls `mergeEntityFields({source:'behavior'})` — writes
