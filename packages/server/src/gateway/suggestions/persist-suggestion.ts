...
// ── Starter suggestions (per agent+org, shown before a conversation starts) ──
//
// Distinct from the per-conversation `suggestion:<conversationId>` rows above:
// a fresh draft has no server conversation yet, so starter chips are keyed per
// AGENT+ORG (`origin_id='starter:<agentId>'`) and generated by a hidden
// "starters" turn that inspects the workspace. They persist across turns (they
// are org-lifecycle-scoped, not conversation-scoped). The starter event's own
// monotonic id is the cache marker; the landing pull regenerates after a later
// non-suggestion org event advances the workspace stream.

export interface PersistStarterArgs {
  organizationId: string;
  agentId: string;
  prompts: SuggestedPrompt[];
  /** The per-run token's `messageId` of the generating starters turn. */
  turnMessageId?: string;
  runId?: number | null;
}

/**
 * Persist the agent+org CURRENT starter set as a superseding
 * `interaction_type='suggestion'` event keyed on `starter:<agentId>`. Same
 * explicit-supersede + advisory-lock machinery as {@link persistSuggestion}
 * (lock keyed on organizationId + agentId). Returns the new event id.
 */
export async function persistStarterSuggestion(
  args: PersistStarterArgs,
): Promise<number> {
  const { organizationId, agentId, prompts, turnMessageId, runId } = args;

  return getDb().begin(async (tx: DbClient) => {
    await tx`SELECT pg_advisory_xact_lock(${SUGGESTION_LOCK_NAMESPACE}, ${suggestionLockKey(
      `${organizationId}:starter:${agentId}`,
    )})`;
    ...
    logger.info(
      `Persisted starter event ${event.id} for agent ${agentId}` +
        (supersedesEventId ? ` (superseded ${supersedesEventId})` : ""),
    );
    return Number(event.id);
  });
...
/**
 * Read the agent+org current starter set. Returns null if none is current.
 */
export async function readCurrentStarter(
  organizationId: string,
  agentId: string,
  sql: DbClient = getDb(),
): Promise<{
...
/**
 * The org's latest non-suggestion event id. Suggestion events are derived UI
 * state, so allowing them to advance this marker would make one agent's chip
 * generation invalidate every other agent's starter cache.
 */
export async function currentOrgEventsMarker(
  organizationId: string,
  sql: DbClient = getDb(),
): Promise<number> {
  const rows = (await sql`
    SELECT COALESCE(MAX(id), 0)::bigint AS marker
    FROM events
    WHERE organization_id = ${organizationId}
      AND interaction_type <> 'suggestion'
  `) as Array<{ marker: number | string }>;
  return Number(rows[0]?.marker ?? 0);
}

/**
 * Is the cached starter set stale relative to the org's latest workspace event?
 *
 * Stale when there is no cache, or the org has appended a non-suggestion event
 * after the starter event itself.
 */
export function isStarterStale(
  cached: { id: number } | null,
  currentMarker: number,
): boolean {
  if (!cached) return true;
  return currentMarker > cached.id;
}
