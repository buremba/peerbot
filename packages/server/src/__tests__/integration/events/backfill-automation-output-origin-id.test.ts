/**
 * The one-off backfill that re-stamps Automation event-output rows written with a
 * POSITIONAL origin_id (`automation_<id>_<output>_canvas_<revision>_<index>`).
 *
 * The rows this repairs are NOT duplicates: prod Automation 71 wrote 8 distinct
 * posts under `automation_71_signals_canvas_4819569_4`, each correctly deduped by
...
import { backfillAutomationOutputOriginId } from '../../../events/backfill-automation-output-origin-id';
import { cleanupTestDatabase, getTestDb } from '../../setup/test-db';
import { TestWorkspace } from '../../setup/test-mcp-client';

describe('backfillAutomationOutputOriginId', () => {
  beforeEach(async () => {
  ...
  it('gives each collided row its own origin_id and leaves positional rows alone', async () => {
    ...
    // Three runs of Automation 71 appending to one Canvas revision, each ranking a
    ...
    for (const row of collided) {
      const inserted = await insert(
        'automation_71_signals_canvas_4819569_4',
        row.title,
        `automation:71:output:signals:key:${row.sourceKey}`
      );
    ...
    const positional = await insert(
      'automation_71_drafts_canvas_4819569_0',
      'Unkeyed draft',
      'automation:71:canvas:4819569:output:drafts:item:0'
    );

    const dry = await backfillAutomationOutputOriginId({ db: sql, execute: false, sleepMs: 0 });
    ...
    const run1 = await backfillAutomationOutputOriginId({ db: sql, execute: true, sleepMs: 0 });
    ...
    expect(repaired.map((r) => r.origin_id)).toEqual(
      collided.map((r) => `automation_71_signals_key_${r.sourceKey}`)
    );

    const untouched = await sql<{ origin_id: string }>`
      SELECT origin_id FROM events WHERE id = ${positional[0].id}
    `;
    expect(untouched[0].origin_id).toBe('automation_71_drafts_canvas_4819569_0');

    // Resumable: a second pass re-derives the same names and writes nothing.
    const run2 = await backfillAutomationOutputOriginId({ db: sql, execute: true, sleepMs: 0 });
  ...
  it('rejects knobs that would make the backfill silently no-op', async () => {
    const sql = getTestDb();
    await expect(
      backfillAutomationOutputOriginId({ db: sql, batchSize: 0, sleepMs: 0 })
    ).rejects.toThrow(/batchSize must be a positive integer/);
    await expect(
      backfillAutomationOutputOriginId({ db: sql, sleepMs: -1 })
