/**
 * Per-agent tool policy: which tools an agent may use, and the bash
 * prefix allow/deny lists derived from the same patterns.
 *
 * Shared by every lane that assembles an agent's tool list or runs its shell —
 * the agent worker, the gateway producer that hands an isolate turn its tools,
 * and the isolate guest that runs the turn's bash — so one agent gets one
 * answer whichever lane runs the turn. Everything here is pure: the isolate
 * guest imports this module by its own subpath (`@lobu/core/tool-policy`),
 * so it must never grow a Node import or a root `@lobu/core` import.
 */
...
import type { ToolsConfig } from "./contracts/agent-settings";
...
export const DEFAULT_PACKAGE_MANAGER_DENY_PREFIXES = [
  "apt ",
  "apt-get ",
  "yum ",
  "dnf ",
 ...
 * preflight (`runtime/tools.ts`) and the isolate guest's workspace `bash`.
 *
 ...
 *
 * Those misses are not holes on either shell that runs today. On the agent
 * worker's default local backend the manager is not a runnable command:
 * `UNSANDBOXED_INTERPRETERS` in the worker's `embedded/just-bash-bootstrap.ts`
 * keeps it out of the just-bash registry, and that lookup is on the RESOLVED
 * binary name, which quoting and paths cannot change. The isolate guest's
 * workspace has no package manager registered at all. Either way the
 * spellings above end in "command not found"; declared `nixPackages` is the
 * sanctioned way to get tooling.
 *
 * Remote runtime providers (opt-in, e.g. `vercel`) have no discovery filter,
 * but the worker's preflight runs before dispatch, so these deny prefixes
 * apply there too and are what keeps a manager from reaching the provider at
 * all. Loosening that for the sandbox tier would be a deliberate, separate
 * change.
 *
