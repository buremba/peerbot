 ...
 * `entities.field_controls` on the behavior PROMOTION path, but the direct
 ...
 * runs an ownership-aware `source:'behavior'` merge: unowned fields write
...
/** Seed a behavior + canvas window (real FKs for behavior_source / reactions). */
async function seedBehaviorAndWindow(workspace: TestWorkspace, suffix: string) {
  ...
  const behavior = (await workspace.owner.behaviors.create({
    entity_id: entity.id,
    slug: `gate-behavior-${suffix}`,
    name: `Gate Behavior ${suffix}`,
...
describe('ownership gate on agent entity writes', () => {
  ...
  it('attributes the proposal to the behavior on the reaction path', async () => {
    const org = workspace.org.id;
    const user = workspace.users.owner.id;
    const {
      ...
      windowId,
      agentId,
    } = await seedBehaviorAndWindow(workspace, 'reaction');
    ...
    // The reaction acts as its OWN behavior: the explicit behavior_source names a
    // behavior owned by this agent, so the tag is honored (an agent can only tag
    // its own behavior — a foreign tag is ignored, see resolveActingPrincipal).
    const result = await manageEntityUpdate(
      agentCtx(org, user, agentId),
      reactionEntity.id,
      { severity: 'critical' },
      { behavior_source: { behavior_id: behaviorId, window_id: windowId } }
    );
    ...
    `;
    expect(run).toBeTruthy();
    expect(Number((run.action_input as { behavior_id: number }).behavior_id)).toBe(behaviorId);

    // The card attribution flows through as 'behavior' so the SPA labels it
    // "A behavior proposes…" instead of "An agent proposes…".
    expect(result.approval_attribution).toBe('behavior');
  });

  it("ignores a caller-supplied behavior_source that is NOT the acting agent's own behavior", async () => {
    const org = workspace.org.id;
    const user = workspace.users.owner.id;
    // A behavior owned by a DIFFERENT agent than the one making the call.
    const {
      entity: target,
      behaviorId,
      windowId,
    } = await seedBehaviorAndWindow(workspace, 'foreign');

    await manageEntityUpdate(humanCtx(org, user), target.id, {
      severity: 'high',
    });

    // A restricted agent tags the foreign behavior, trying to escape its own
    // envelope. The tag must be IGNORED → the write is attributed to the agent,
    // not the behavior (ownerAgentId can't be nulled out via a foreign tag).
    const result = await manageEntityUpdate(
