...
- `egress-policy.ts` (`@lobu/connector-sdk/egress-policy`) is the ONE egress pattern grammar and decision order for gateway-mediated egress: the gateway proxy, the grant and policy stores and the remote runtime provider all match hosts through it. It is pure (no `node:`, no `@lobu/core`) so the same code runs on the Node host, inside an isolate, and on workerd. Change the grammar here or nowhere; configured patterns are normalized once by `@lobu/core`'s `normalizeDomainPattern`. The connector isolate lane is the one egress path still on its own matcher (`hostAllowed` in `@lobu/connector-worker`); converging it narrows what a run may reach, so it is its own PR.
