...
exports.runClaudeWithProgress = runClaudeWithProgress;
...
function parseCustomEnvVars(claudeEnv) {
...
function prepareRunConfig(promptPath, options) {
    ...
    if (options.timeoutMinutes) {
        ...
        if (isNaN(timeoutMinutesNum) || timeoutMinutesNum <= 0) {
            throw new Error(`timeoutMinutes must be a positive number, got: ${options.timeoutMinutes}`);
        }
    }
...
async function runClaudeWithProgress(promptPath, options, onProgress) {
    const config = prepareRunConfig(promptPath, options);
    // Create a named pipe
    try {
        await (0, promises_1.unlink)(PIPE_PATH);
    }
    catch (e) {
    ...
    try {
        const stats = await (0, promises_1.stat)(config.promptPath);
    ...
    const catProcess = (0, child_process_2.spawn)("cat", [config.promptPath], {
        stdio: ["ignore", "pipe", "inherit"],
    });
    const pipeStream = (0, fs_1.createWriteStream)(PIPE_PATH);
    catProcess.stdout.pipe(pipeStream);
    catProcess.on("error", (error) => {
        console.error("Error reading prompt file:", error);
        pipeStream.destroy();
    });
    const claudeProcess = (0, child_process_2.spawn)("claude", config.claudeArgs, {
    ...
    const pipeProcess = (0, child_process_2.spawn)("cat", [PIPE_PATH]);
    ...
    if (exitCode === 0) {
        try {
            await (0, promises_1.writeFile)("output.txt", output);
            // Process output.txt into JSON and save to execution file
            const { stdout: jsonOutput } = await execAsync("jq -s '.' output.txt");
            await (0, promises_1.writeFile)(EXECUTION_FILE, jsonOutput);
...
//# sourceMappingURL=claude-execution.js.map