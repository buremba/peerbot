...
8. **Memory data deferred to v3**. v1 ships memory **schema** only (entity + relationship types via existing admin tools). Automations, entities, relationships, knowledge are out.
...
6. Automation fallback to "first seeded entity" when ref unresolvable (apply doesn't sync automations in v1, but the principle: never invent a target).
