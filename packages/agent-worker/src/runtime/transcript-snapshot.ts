 ...
 * successful worker run from `LobuAgentWorker.execute()`, before terminal
 * acknowledgement. The
 ...
 *
 * Returns whether the completed snapshot is durable. The worker uses this
 * result as a completion barrier: it must not acknowledge the run before the
 * transcript containing every accepted steering input is persisted.
 */
export async function writeSnapshot(
  ...
  try {
    ...
    if (!res.ok) {
      // A 409 means the existing same-run transcript is not a prefix of this
      // one, so the gateway refused to overwrite a divergent branch. It is
      // NOT a completed durability checkpoint: acknowledging here would bind
      // terminal delivery to stale transcript bytes.
