...
exports.createPromptFile = createPromptFile;
exports.createSimplePromptFile = createSimplePromptFile;
 ...
 */
function generateContextSection(context) {
    const sections = [];
    ...
    if (context.threadTs) {
        sections.push("Session Type: Thread-based conversation (resuming previous discussion)");
    }
    else {
 ...
 */
function generateEnvironmentSection(context) {
    const sections = [];
    sections.push("## Working Environment");
    if (context.repositoryUrl) {
        sections.push("You are working in a user-specific GitHub repository:");
        sections.push(`- Repository: ${context.repositoryUrl}`);
        sections.push(`- Working Directory: ${context.workingDirectory || '/workspace'}`);
 ...
 */
async function createPromptFile(context, conversation = []) {
    const promptParts = [];
    ...
    const promptPath = (0, path_1.join)(TEMP_DIR, `claude-prompt-${Date.now()}.md`);
    await (0, promises_1.writeFile)(promptPath, promptContent, "utf-8");
 ...
 */
async function createSimplePromptFile(userRequest) {
...
Please provide a helpful and concise response.`;
    const promptPath = (0, path_1.join)(TEMP_DIR, `claude-simple-prompt-${Date.now()}.md`);
    await (0, promises_1.writeFile)(promptPath, promptContent, "utf-8");
    console.log(`Created simple prompt file: ${promptPath}`);
    return promptPath;
}
//# sourceMappingURL=prompt-generation.js.map