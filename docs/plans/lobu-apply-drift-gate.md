...
- **`owned`** — the set of **definition incarnation IDs** (persistent per-row ids: `entity_types.id`, `behavior_id`, `connector_definitions.id`) **this config actually applied** (delete-eligible). Storing IDs, not slugs or connector key+version, is load-bearing: a config-expressed delete that succeeds and then has its summary POST fail leaves the id in `owned`; if the user re-creates the same slug (or re-installs the same connector key/version — uninstall archives the row, so a reinstall is a **new `connector_definitions.id`**) in the UI, the new row has a **new id** and is therefore **not owned** — it blocks as drift instead of being auto-deleted under `--yes`. Slug+value equality alone cannot distinguish "the definition I applied, now removed" from "a UI recreation of it."
