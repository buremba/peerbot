/**
 * Shared helpers for behavior reaction queries and tracking.
 */
...
/**
 * Build a human-readable summary of past behavior reactions.
 */
export async function getPastReactionsSummary(
  behaviorId: number | string,
  limit = 20
): Promise<string | undefined> {
  const sql = getDb();
  // behavior_reactions.window_id is the canvas ROOT event id; the canvas_windows
  ...
  const reactions = await sql`
    ...
    FROM behavior_reactions wr
    LEFT JOIN canvas_windows ww ON ww.id = wr.window_id
    WHERE wr.behavior_id = ${behaviorId}
...
/**
 * Track a behavior reaction (fire-and-forget safe).
 */
export async function trackBehaviorReaction(params: {
  organizationId: string;
  behaviorId: number;
  windowId: number;
  reactionType: string;
  ...
  const sql = getDb();
  await sql`
    INSERT INTO behavior_reactions (
      organization_id, behavior_id, window_id,
      reaction_type, tool_name, tool_args, tool_result, entity_id, run_id
    ) VALUES (
      ${params.organizationId}, ${params.behaviorId}, ${params.windowId},
  ...
  `.catch((err) => logger.error(err, 'Failed to track behavior reaction'));
}
