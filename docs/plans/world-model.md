...
- `venture-capital` — entity content was fully duplicated in `market`
  ...
  Dropped: org row + 12,504 events + 13 founder automations + 55 connections
  + 78 feeds + 8,271 runs.
- `careops` (Healthcare) — out of scope; dropped the workspace, agent,
  automations, connections, and the seed entities (1 patient, 1 appointment,
...
- `market-intelligence` — folded into `market`. All 21 brand entities
  ...
  the company rows; 275,147 events, 9 automations, 78 connections, 84 feeds,
  ...
  rewrite. Entity_ids[] arrays in events/automations/feeds rewritten via
...
Followup: events + automations have no FK to organization, so the org drop
...
- A way for the admin agent to declare which entity types it's interested
  in (could be a `requires_review` automation in the admin's public catalog).
