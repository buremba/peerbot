...
- **Capture (after the session).** When `autoCapture` is on, the plugin distills the conversation into observations and `save_memory`s them, stamping `metadata.agent_id` when `agentId` is set so each agent's writes stay attributable.

## Tools the agent gets

Once connected, the agent can call the Lobu memory MCP tools directly:
