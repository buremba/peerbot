/**
 * The mutation gate must fail CLOSED inside behavior promotion: a `deny` from
...
async function setup() {
  ...
  `;
	...
	// A REAL behavior (with an owning agent) — the gate resolves behaviors.agent_id and
	// FAILS CLOSED if the behavior row is missing, so a fake id would now deny every
	// write. We exercise the interceptor fail-closed here, not owner-resolution.
	const agent = await createTestAgent({
		organizationId: org.id,
		ownerUserId: createdBy,
	});
	// behaviors.behavior_group_id is NOT NULL and self-references the row (the CRUD sets
	// it = the behavior id); id is serial. Grab the next id first so both match.
	const [{ nextid }] = await sql<{ nextid: number }>`
    SELECT nextval('behaviors_id_seq') AS nextid
  `;
	const behaviorId = Number(nextid);
	await sql`
    INSERT INTO behaviors (id, organization_id, slug, name, agent_id, created_by, behavior_group_id, created_at, updated_at)
    VALUES (${behaviorId}, ${org.id}, 'gate-fail-closed', 'Gate Fail Closed', ${agent.agentId}, ${createdBy}, ${behaviorId}, NOW(), NOW())
  `;
	return { sql, orgId: org.id, parentId: parent.id, createdBy, behaviorId };
}
...
async function promotedIdentities(ctx: Awaited<ReturnType<typeof setup>>) {
    ...
    WHERE ei.organization_id = ${ctx.orgId} AND ei.namespace = 'behavior_key'
  `;
}

describe("mutation gate fail-closed in behavior promotion", () => {
	beforeAll(async () => {
