...
function outName(kind: string, name: string): string {
	if (!SANE_IDENT.test(name)) {
		throw new MetricCompileError(
			`${kind} name "${name}" must be a plain identifier`,
		);
	}
	return name;
}
...
export function compileMetricSql(input: CompileMetricInput): string {
	...
	if (!eventSet.field) {
		throw new MetricCompileError(
			`alias eventSet "${measure.eventSet}" needs a "field"`,
		);
	}
	...
	const dims = (input.by ?? []).map((name) => {
		...
		if (!dim)
			throw new MetricCompileError(`dimension "${name}" is not declared`);
	...
	const entWhere = [
		`ent.entity_type_id = ${Number(entityTypeId)}`,
		`ent.deleted_at IS NULL`,
	];
	if (input.entityId !== undefined)
		entWhere.push(`ent.id = ${Number(input.entityId)}`);
...
export function compileDerivedMetricSql(input: {
	...
	const columns = new Map(
		inferColumns(input.backingSql).map((column) => [column.name, column.role]),
	);
	if (columns.get(input.measure) !== "measure") {
		throw new MetricCompileError(
			`measure "${input.measure}" is not declared or inferred`,
		);
	}
