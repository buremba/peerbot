 ...
 *
 * Handles:
 * 1. Creating automation classifiers from template definitions
 * 2. Extracting values from automation data using JSONPath
...
/**
 * Extract classifier values from automation data
 */
...
/**
 * Create classifiers for an automation from template definitions
 */
export async function createClassifiersForAutomation(
	sql: DbClient,
	automationId: number,
	entityId: number,
	...
	options: {
		...
		// tenant-less classifier. The sole caller (manage_automations/crud.ts)
	...
	for (const def of classifierDefs) {
      ...
      ON CONFLICT (organization_id, entity_id, automation_id, slug) DO UPDATE
    ...
    `;
		...
		logger.info(
			`[ClassifierExtraction] Created/updated classifier "${def.slug}" for automation ${automationId}`,
		);
	}
...
/**
 * Update classifier values after automation extraction
...
async function updateClassifierValues(
	...
	// Get classifiers for this automation with extraction config. persistAttributeValues writes learned
  ...
  `;
	...
	for (const classifier of classifiers as any[]) {
		...
		// Extract new values from automation data
...
/**
 * Process automation classifications: create LLM-based classifications from extracted data.
 *
...
export async function processAutomationClassifications(
	...
	try {
		...
		await updateClassifierValues(sql, automationId, extractedData, env);
