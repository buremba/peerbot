/**
 * One-off backfill: populate `automations.reaction_input_schema` for reactions that
 ...
 * `automations.reaction_input_schema`, so the worker is told the exact shape and the
 ...
 *
 * `reaction_script` / `reaction_input_schema` live on `automations` and are written
 * group-wide by `automation_group_id` (the same cascade set_reaction_script uses). So
 * one representative automation per group with a NULL schema is re-extracted and the
...
export async function backfillReactionInputSchema(opts: {
  ...
  for (const g of groups) {
    ...
    if (!schema) {
      noInput++;
      log(`  group ${g.automation_group_id} (org ${g.organization_id}): no \`input\` export — leaving NULL`);
      continue;
    }
    filled++;
    const keys = Object.keys((schema.properties as Record<string, unknown>) ?? {});
    log(
      `  group ${g.automation_group_id} (org ${g.organization_id}): input schema [${keys.join(', ')}]` +
    ...
    if (execute) {
      await sql`
        ...
        WHERE automation_group_id = ${g.automation_group_id}
          AND reaction_input_schema IS NULL
      `;
    }
  }
