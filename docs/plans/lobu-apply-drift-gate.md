...
10. `--only` scope: baseline A → `lobu apply --only memory` applies config B (succeeds; memory family advanced, agents family carried forward) → change config to C → a full `lobu apply` **converges C normally** instead of blocking it as both-moved; assert the scoped run is in deployment history. Repeat with `--only agents`.
