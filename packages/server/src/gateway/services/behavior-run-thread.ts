...
/**
 * Read the latest N durable behavior runs for one behavior, newest first, ready
 ...
 */
export async function readBehaviorRunThreads(args: {
	agentId: string;
	behaviorId: number;
	...
	const { agentId, behaviorId, organizationId, limit } = args;
	const sql = getDb();
	const rows = await sql<{
		...
		WITH selected_runs AS (
			...
			FROM runs r
			JOIN behaviors w
			  ON w.id = r.behavior_id
			 AND w.organization_id = r.organization_id
			WHERE r.organization_id = ${organizationId}
			  AND r.behavior_id = ${behaviorId}
		...
		LEFT JOIN behavior_versions version
	...
	const runs = rows.map((row) => {
		...
		const messages = row.snapshot_jsonl
			? paginateSessionMessages(row.snapshot_jsonl, "", 200, {
					excludeVerbose: true,
					sessionIdFallback: row.conversation_id ?? `behavior-run-${runId}`,
				}).messages
			: [];
		return {
			runId,
	...
	// them separately, then attach each to its producing behavior run by durable
