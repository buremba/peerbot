...
4. `IsolateExecutor` defaults to a 10-minute wall clock and a 512 MB heap; the standalone daemon raises the heap, and interactive auth runs disable the fixed timeout while waiting for the user.
5. The guest has no filesystem, no sockets, and no ambient environment: everything it can reach is a named capability the host granted, plus the explicit values supplied in `job.env`.
