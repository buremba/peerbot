...
function initializeMetrics() {
  ...
  // Scheduler + behavior-automation health. These back the prod alerting rules
  // (charts/lobu PrometheusRule): a silent scheduler / failing behavior tick is
  ...
  registerMetric(
    "lobu_behavior_automation_phase_failures_total",
    "behavior-automation phases that threw, by phase (reset|reconcile|materialize|dispatch)",
    "counter"
  );
  registerMetric(
    "lobu_behavior_runs_created_total",
    "Behavior runs materialized (enqueued) by the scheduler",
    "counter"
  );
  registerMetric(
    "lobu_behaviors_unrunnable",
    "Due active behaviors skipped this tick for lacking a runnable executor (no device pin, no agent row)",
    "gauge"
  );
