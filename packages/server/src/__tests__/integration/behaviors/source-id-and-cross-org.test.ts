 ...
 *  time. Behavior-mode aggregation keys rows by `id` and the signed window_token
 ...
 *  A behavior attached to a foreign entity links its content to a non-existent
 *  in-org entity.
 */
...
describe("manage_behaviors source-id + cross-org guards", () => {
	...
	beforeAll(async () => {
		...
		const org = await createTestOrganization({ name: "Behavior Guard Org" });
		ownerOrgId = org.id;
		const user = await createTestUser({ email: "behavior-guard@test.com" });
		...
		const foreignOrg = await createTestOrganization({
			name: "Behavior Foreign Org",
		});
		const foreignEntity = await createTestEntity({
			name: "Foreign Behavior Target",
			entity_type: "company",
	...
	it("resolves source refs and only signs event-backed content ids", async () => {
		...
		const connection = await createTestConnection({
			...
			display_name: "Behavior Source Ref Connection",
			slug: "behavior-source-ref-connection",
		});
    ...
    `;
		...
		// Regression guard: the old stats query counted only behavior.entity_ids, so
		// an org-scoped @feed behavior returned content but reported total_count=0.
    ...
    `;
		...
		// No behavior leaked pointing at the foreign entity.
	...
	// requireBehaviorAccess lets an id that is absent from the caller's org fall
	// through to the per-id aggregate (so genuinely-missing ids report "not
	// found"). Behavior ids are sequential integers, so a foreign-tenant row can
	...
	it("delete cannot archive a behavior owned by another org", async () => {
		...
		const foreignBehavior = (await foreignClient.behaviors.create({
		...
		const foreignId = String(foreignBehavior.behavior_id);

		// The owner (a different org) targets the foreign row's id directly. It
		// exists under another tenant, so requireBehaviorAccess rejects it 403 —
      ...
      SELECT status FROM behaviors WHERE id = ${foreignId}
    `;
