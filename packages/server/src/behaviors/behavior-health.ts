 ...
 *
 * The behavior reaper (behaviors/automation.ts) already times out stuck pending
 ...
 * missed a firing. The behavior scheduler (reconcileBehaviorRuns) runs on a
 ...
 *  back to 2h (the BEHAVIOR_RUN_STALE_INTERVAL default). */
...
export interface BehaviorHealthInput {
  ...
  /** `behaviors.next_run_at` — the scheduler cursor. */
  nextRunAt: string | Date | null | undefined;
  /** Latest run status (from buildLatestBehaviorRunJoinSql). */
 ...
 *     (BEHAVIOR_RUN_STALE_INTERVAL) — a stuck pending run the reaper hasn't
 *     timed out yet.
 * Otherwise `healthy`.
 */
export function computeBehaviorHealth(
  ...
  if (input.latestRunStatus === 'pending') {
    ...
    const staleMs = pgIntervalToMs(intervals.behaviorRunStaleInterval);
