 ...
 *   - Trigger does NOT advance `behaviors.next_run_at`.
 */
...
/**
 * Set up a device-pinned behavior owned by the workspace owner. Returns
 * everything the tests need to mint the right PAT + assert on the run row.
 */
async function setupDevicePinnedBehavior(opts: { workerId: string }): Promise<{
  ...
  // Pin the behavior to the device. `BehaviorCreateInput` doesn't expose
  // device_worker_id / agent_kind, so set them directly — matches how
  // automation-contract.test.ts pins behaviors for the #802 dispatcher tests.
  await sql`
    UPDATE behaviors
...
describe('POST /api/workers/me/behaviors/:behavior_id/trigger', () => {
  beforeEach(async () => {
    await cleanupTestDatabase();
  });

  it('correctly-bound device → 200 + manual run row', async () => {
    const ctx = await setupDevicePinnedBehavior({ workerId: 'mac-trigger-ok' });
    ...
    const response = await post(`/api/workers/me/behaviors/${ctx.behaviorId}/trigger`, { token });
    ...
    const runs = await ctx.sql`
      SELECT id, status, behavior_id, run_type, approved_input
      FROM runs
      WHERE behavior_id = ${ctx.behaviorId}
    `;
  ...
  it('wrong device → 403, no run created', async () => {
    const ctx = await setupDevicePinnedBehavior({ workerId: 'mac-pinned' });
    const ownerUserId = ctx.workspace.users.owner.id;

    // Same user registers a second, unrelated device. Their token is bound
    // to that second worker_id and must NOT be able to trigger behavior A.
    await ctx.sql`
      INSERT INTO device_workers (user_id, worker_id, platform, capabilities, label, organization_id)
    ...
    const runs = await ctx.sql`
      SELECT id FROM runs WHERE behavior_id = ${ctx.behaviorId}
    `;
    expect(runs).toHaveLength(0);
  });

  it('re-trigger while a run is pending → 200 already_queued, no duplicate run', async () => {
    const ctx = await setupDevicePinnedBehavior({
    ...
    const first = await post(`/api/workers/me/behaviors/${ctx.behaviorId}/trigger`, { token });
    ...
    const second = await post(`/api/workers/me/behaviors/${ctx.behaviorId}/trigger`, { token });
  ...
  it('also returns already_queued for claimed/running existing runs', async () => {
    ...
    const runs = await ctx.sql`
      SELECT id, status FROM runs WHERE behavior_id = ${ctx.behaviorId}
    `;
    expect(runs).toHaveLength(1);
    expect(String(runs[0].status)).toBe('running');
  });

  it('does NOT advance behaviors.next_run_at', async () => {
    const ctx = await setupDevicePinnedBehavior({ workerId: 'mac-trigger-nra' });
    const { token } = await createWorkerBoundPat(
      ctx.workspace.users.owner.id,
      ctx.workspace.org.id,
      'mac-trigger-nra'
    ...
    const [before] = await ctx.sql`
      SELECT next_run_at FROM behaviors WHERE id = ${ctx.behaviorId}
    `;
  ...
  it('returns 404 for an unknown behavior id', async () => {
    const ctx = await setupDevicePinnedBehavior({ workerId: 'mac-404' });
    const { token } = await createWorkerBoundPat(
      ctx.workspace.users.owner.id,
      ctx.workspace.org.id,
      'mac-404'
  ...
  it('poll payload carries the behavior execution_config', async () => {
    const ctx = await setupDevicePinnedBehavior({ workerId: 'mac-poll-exec' });
    const execCfg = {
    ...
    await ctx.sql`
      UPDATE behaviors SET execution_config = ${ctx.sql.json(execCfg)} WHERE id = ${ctx.behaviorId}
    `;
    const { token } = await createWorkerBoundPat(
      ctx.workspace.users.owner.id,
      ctx.workspace.org.id,
    ...
    const trig = await post(`/api/workers/me/behaviors/${ctx.behaviorId}/trigger`, { token });
    ...
    const job = (await pollRes.json()) as {
      run_type?: string;
      payload?: { behavior?: { execution_config?: Record<string, unknown> } };
    };
    expect(job.run_type).toBe('behavior');
    expect(job.payload?.behavior?.execution_config).toEqual(execCfg);
  });

  it('leaves a second queued run pending while the same Behavior is running', async () => {
    const ctx = await setupDevicePinnedBehavior({ workerId: 'mac-poll-serial' });
    const { token } = await createWorkerBoundPat(
      ctx.workspace.users.owner.id,
      ctx.workspace.org.id,
      'mac-poll-serial'
  ...
  it('serializes concurrent claims for two pending runs of one Behavior', async () => {
    const ctx = await setupDevicePinnedBehavior({ workerId: 'mac-poll-race' });
    const { token } = await createWorkerBoundPat(
      ctx.workspace.users.owner.id,
      ctx.workspace.org.id,
      'mac-poll-race'
    ...
    const triggered = await post(`/api/workers/me/behaviors/${ctx.behaviorId}/trigger`, {
      token,
    });
  ...
  it('poll payload carries the run-pinned prompt and skills, not a later edit', async () => {
    const ctx = await setupDevicePinnedBehavior({ workerId: 'mac-poll-prompt' });
    const { token } = await createWorkerBoundPat(
      ctx.workspace.users.owner.id,
      ctx.workspace.org.id,
      'mac-poll-prompt'
    ...
    await ctx.sql`
      ...
      WHERE id = (
        SELECT current_version_id FROM behaviors WHERE id = ${ctx.behaviorId}
      )
    `;

    // Queue a run: createBehaviorRun snapshots the current version (prompt +
    // skills v1) into approved_input.version_id.
    const trig = await post(`/api/workers/me/behaviors/${ctx.behaviorId}/trigger`, { token });
    expect(trig.status).toBe(200);

    // Edit the behavior AFTER the run is queued: a new current version with a
    ...
    const job = (await pollRes.json()) as {
      run_type?: string;
      payload?: { behavior?: { prompt?: string } };
    };
    expect(job.run_type).toBe('behavior');
    expect(job.payload?.behavior?.prompt).toContain('Summarize {{entities}}.');
    expect(job.payload?.behavior?.prompt).toContain('### device-runbook');
    expect(job.payload?.behavior?.prompt).toContain('Follow the frozen v1 runbook.');
    expect(job.payload?.behavior?.prompt).not.toContain('EDITED-AFTER-QUEUE');
  });
});
