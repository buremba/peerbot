 ...
 *
 * Handles:
 * 1. Creating behavior classifiers from template definitions
 * 2. Extracting values from behavior data using JSONPath
 * 3. Normalizing values using embedding similarity (duplicate detection)
 * 4. LLM-based content classification from behaviors (via processExtractedClassifications)
 */
...
/**
 * Extract classifier values from behavior data
 */
...
/**
 * Create classifiers for an behavior from template definitions
 */
export async function createClassifiersForBehavior(
	sql: DbClient,
	behaviorId: number,
	entityId: number,
	...
	for (const def of classifierDefs) {
      ...
      ON CONFLICT (organization_id, entity_id, behavior_id, slug) DO UPDATE
    ...
    `;
		...
		logger.info(
			`[ClassifierExtraction] Created/updated classifier "${def.slug}" for behavior ${behaviorId}`,
		);
	}
...
/**
 * Update classifier values after behavior extraction
...
async function updateClassifierValues(
	...
	// Get classifiers for this behavior with extraction config. persistAttributeValues writes learned
  ...
  `;
	...
	for (const classifier of classifiers as any[]) {
		...
		// Extract new values from behavior data
...
async function processExtractedClassifications(
	...
	for (const classifier of classifiers) {
		...
		for (const item of items) {
            ...
            ON CONFLICT (event_id, classifier_id, source, COALESCE(behavior_id, 0))
            DO UPDATE SET
	...
	if (totalClassifications > 0) {
		logger.info(
			{ behaviorId, windowId, totalClassifications, invalidCitations },
	...
	if (invalidCitations > 0) {
		logger.warn(
			{ behaviorId, windowId, invalidCitations },
...
/**
 * Process behavior classifications: create LLM-based classifications from extracted data.
 *
 ...
 */
export async function processBehaviorClassifications(
	sql: DbClient,
	behaviorId: number,
	windowId: number,
	extractedData: any,
	...
	try {
		...
		await updateClassifierValues(sql, behaviorId, extractedData, env);
		...
		logger.error(
			{ error, behavior_id: behaviorId, window_id: windowId },
