...
class SessionManager {
    ...
    timeoutMinutes;
    constructor(config) {
        this.gcsStorage = new gcs_1.GcsStorage(config);
    ...
    validateSessionKey(sessionKey) {
        if (!sessionKey || typeof sessionKey !== 'string') {
            throw new types_1.SessionError(sessionKey, "INVALID_KEY", "Session key must be a non-empty string");
        }
        if (sessionKey.length > 100) {
            throw new types_1.SessionError(sessionKey, "INVALID_KEY", "Session key too long (max 100 characters)");
        }
        ...
        for (const pattern of maliciousPatterns) {
            if (pattern.test(sessionKey)) {
                throw new types_1.SessionError(sessionKey, "INVALID_KEY", "Session key contains invalid characters or patterns");
            }
        }
        // Allow only alphanumeric, dots, hyphens, and underscores
        if (!/^[a-zA-Z0-9._-]+$/.test(sessionKey)) {
            throw new types_1.SessionError(sessionKey, "INVALID_KEY", "Session key must contain only alphanumeric characters, dots, hyphens, and underscores");
        }
    }
    /**
     * Create a new session
     */
    async createSession(sessionKey, context) {
    ...
    async recoverSession(sessionKey) {
        ...
        try {
            ...
            throw new types_1.SessionError(sessionKey, "RECOVERY_FAILED", `Failed to recover session from GCS`, error);
        }
    }
    /**
    ...
    async addMessage(sessionKey, message) {
        ...
        if (!session) {
            throw new types_1.SessionError(sessionKey, "SESSION_NOT_FOUND", "Session not found");
        }
     ...
     */
    async updateProgress(sessionKey, update) {
        this.validateSessionKey(sessionKey);
        const session = this.activeSessions.get(sessionKey);
        if (!session) {
     ...
     */
    startTimeoutMonitoring(sessionKey) {
        this.validateSessionKey(sessionKey);
    ...
    async persistSession(sessionKey) {
        ...
        const session = this.activeSessions.get(sessionKey);
        if (!session) {
            throw new types_1.SessionError(sessionKey, "SESSION_NOT_FOUND", "Session not found for persistence");
        }
        try {
            ...
            throw new types_1.SessionError(sessionKey, "PERSISTENCE_FAILED", "Failed to persist session to GCS", error);
        }
    }
    /**
     * Check if session exists in GCS
     */
    async sessionExistsInGcs(sessionKey) {
        this.validateSessionKey(sessionKey);
    ...
    async cleanupAll() {
        ...
        const promises = Array.from(this.activeSessions.keys()).map(sessionKey => this.cleanup(sessionKey).catch(error => console.error(`Failed to cleanup session ${sessionKey}:`, error)));
