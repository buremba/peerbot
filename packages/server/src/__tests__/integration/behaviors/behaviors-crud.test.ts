/**
 * Behavior CRUD via the post-#348 SDK surface.
 *
 * Replaces the deleted manage_behaviors integration tests. Covers create,
 * read, update, delete on behaviors attached to an entity, plus access-control
...
describe('behavior CRUD', () => {
  ...
  beforeAll(async () => {
    ...
    const org = await createTestOrganization({ name: 'Behavior Test Org' });
    const user = await createTestUser({ email: 'behavior-owner@test.com' });
    ...
    const otherOrg = await createTestOrganization({
      name: 'Behavior Other Org',
    });
    const otherUser = await createTestUser({ email: 'behavior-other@test.com' });
  ...
  it('creates → reads back → updates → deletes a behavior', async () => {
    const created = (await owner.behaviors.create({
      entity_id: entityId,
      slug: 'lifecycle-behavior',
    ...
    const got = (await owner.behaviors.get({ behavior_id: behaviorId })) as {
      behavior?: { behavior_name: string };
    };
    expect(got.behavior?.behavior_name).toBe('Lifecycle Behavior');

    await owner.behaviors.update({
      behavior_id: behaviorId,
      triggers: [{ kind: 'schedule', cron: '0 10 * * *' }],
    });
    const after = (await owner.behaviors.get({ behavior_id: behaviorId })) as {
    ...
    const listedBehavior = listed.behaviors?.find((behavior) => behavior.behavior_id === behaviorId);
    ...
    expect(list.behaviors?.some((w) => w.behavior_id === behaviorId)).toBe(false);
  });

  it('creates a behavior and its reaction contract atomically', async () => {
    const sql = getTestDb();
    ...
    const created = (await owner.behaviors.create({
      slug: 'atomic-reaction-behavior',
      name: 'Atomic Reaction Behavior',
    ...
    >`
      ...
      FROM behaviors WHERE id = ${created.behavior_id}
    `;
  ...
  it('executes an installed merge reaction and queues exactly one pending, unapplied approval with behavior/window attribution', async () => {
    const sql = getTestDb();
    ...
    if (!template) throw new Error('duplicate-merge behavior template is missing');
    const reactionScript = String(template.detail.reaction_script);

    const created = (await owner.behaviors.create({
      slug: 'reaction-merge-exec-behavior',
      name: 'Reaction Merge Exec Behavior',
    ...
    const [installed] = await sql<{ reaction_script_compiled: string | null }[]>`
      SELECT reaction_script_compiled FROM behaviors WHERE id = ${behaviorId}
    `;
    ...
    >`
      SELECT behavior_id, window_id, approval_status, status
      FROM runs
    ...
    expect(Number(pending[0].behavior_id)).toBe(behaviorId);
  ...
  it('does not create a behavior when its reaction fails compilation', async () => {
    await expect(
      owner.behaviors.create({
        slug: 'invalid-reaction-behavior',
        name: 'Invalid Reaction Behavior',
        prompt: 'Return merge proposals.',
        agent_id: agentId,
    ...
    const [row] = await getTestDb()<{ id: number }[]>`
      SELECT id FROM behaviors WHERE slug = 'invalid-reaction-behavior'
    `;
  ...
  it('concurrent creates of the same slug: one wins, every loser gets the coded 409 (not raw 23505)', async () => {
    ...
    // pass it and race idx_behaviors_org_slug. Fire many at once: exactly one
  ...
  it('concurrent creates of DISTINCT slugs all succeed with unique ids (no PK collision)', async () => {
    ...
    // and collided on the behavior PK even with different slugs.
    const N = 6;
  ...
  it('concurrent group-locked writes on DISTINCT groups do not exhaust the pool (holder starvation)', async () => {
    // withBehaviorGroupLock holders keep a main-pool connection for the whole
    ...
    // concurrent create_from_version calls, each from its OWN source behavior
    ...
    for (let i = 0; i < N; i++) {
      ...
      const [row] = await sql<{ current_version_id: number }[]>`
        SELECT current_version_id FROM behaviors WHERE id = ${base.behavior_id}
      `;
  ...
  it('concurrent create_from_version fan-outs allocate unique ids (no PK collision)', async () => {
    ...
    // only way to fail is the behaviors PK colliding on a shared MAX(id)+1.
    const sql = getTestDb();
  ...
  it('concurrent create_from_version to the SAME entity: one wins, losers get a coded 409 (not raw 23505)', async () => {
    ...
    // idx_behaviors_org_slug. The loser must surface a coded 409, not a raw 23505,
  ...
  it('multi-entity create_from_version is all-or-nothing: a mid-fan-out collision rolls back the earlier target too', async () => {
    ...
    // same call must not leak a half-created behavior. This is the atomicity the
    ...
    // The earlier (fresh) target must have NO behavior — the rollback took it.
  ...
  it('creates an org-scoped behavior without an inline extraction schema', async () => {
    const created = (await owner.behaviors.create({
      slug: 'org-scoped-summary-behavior',
      name: 'Org Scoped Summary Behavior',
  ...
  it('derives sources[] from @-mention tokens in the prompt (no sources sent)', async () => {
    ...
    const created = (await owner.behaviors.create({
      entity_id: entityId,
      slug: 'prompt-derived-sources-behavior',
      name: 'Prompt Derived Sources Behavior',
      prompt,
      agent_id: agentId,
  ...
  it('a version bump does not re-derive sources from the prompt — the caller sends them', async () => {
    ...
    const created = (await owner.behaviors.create({
      entity_id: entityId,
      slug: 'sql-edit-rederive-behavior',
      name: 'SQL Edit Rederive Behavior',
  ...
  it('removing every @-mention chip does not clear sources — `sources: []` does', async () => {
    ...
    const created = (await owner.behaviors.create({
      entity_id: entityId,
      slug: 'clear-sources-on-edit-behavior',
      name: 'Clear Sources On Edit Behavior',
  ...
  it('clears entity promotion when create_version receives keying_config: null', async () => {
    const created = (await owner.behaviors.create({
      entity_id: entityId,
      slug: 'clear-keying-config-behavior',
      name: 'Clear Keying Config Behavior',
    ...
    const [row] = await sql`
      ...
      FROM behaviors w
      JOIN behavior_versions wv ON wv.id = w.current_version_id
  ...
  it('round-trips execution_config through create → list → update', async () => {
    const created = (await owner.behaviors.create({
      entity_id: entityId,
      slug: 'exec-config-behavior',
      name: 'Exec Config Behavior',
      prompt: 'Track things.',
      agent_id: agentId,
      execution_config: {
    ...
    expect(findRow(list, behaviorId)?.execution_config).toEqual({
      timeout_seconds: 1800,
      max_budget_usd: 2.5,
      model: 'opus',
      permission_mode: 'acceptEdits',
    ...
    expect(findRow(after, behaviorId)?.execution_config).toEqual({
      timeout_seconds: 300,
    });
    ...
    expect(findRow(cleared, behaviorId)?.execution_config ?? null).toBeNull();
  ...
  it('creates an org-scoped behavior with no entity_id', async () => {
    const created = (await owner.behaviors.create({
      slug: 'org-scoped-behavior',
      name: 'Org Scoped',
  ...
  it('rejects an org-scoped behavior when there is no organization context', async () => {
    const noOrg = owner.withAuth({ organizationId: null });
    await expect(
      noOrg.behaviors.create({
        slug: 'no-org-behavior',
        name: 'No Org',
        prompt: 'should fail',
        agent_id: agentId,
      })
  ...
  it('blocks cross-org reads and writes for org-scoped behaviors', async () => {
    const created = (await owner.behaviors.create({
      slug: 'cross-org-org-scoped-behavior',
  ...
  it('blocks a member from deleting behaviors (admin-only)', async () => {
    const created = (await owner.behaviors.create({
      entity_id: entityId,
      slug: 'protected-behavior',
      name: 'Protected',
      prompt: 'guarded.',
      agent_id: agentId,
    })) as { behavior_id: string };
  ...
  // Issue #1060: a device pin (behaviors.device_worker_id) runs the behavior's
  ...
  describe('device_worker_id ownership gate', () => {
    ...
    it('rejects re-pinning an existing behavior to a foreign-org device (update)', async () => {
      const created = (await owner.behaviors.create({
        entity_id: entityId,
