 ...
 * connections, behaviors, or auth-profiles — so it never disturbs a device a
 * connection or behavior still depends on. The no-binding predicate is re-checked
...
export async function reapStaleDeviceWorkers(): Promise<{
  ...
  const candidates = (await sql`
    ...
    WHERE last_seen_at < now() - interval '30 days'
      ...
      AND NOT EXISTS (
        SELECT 1 FROM behaviors w WHERE w.device_worker_id = device_workers.id
      )
      AND NOT EXISTS (
