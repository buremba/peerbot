...
function fakeClient() {
	...
	return {
		...
		completeAutomation: async (runId: number, value: unknown) =>
			calls.completeAutomation.push({ runId, value }),
	} as never;
}
...
describe("native bridge run forwarding", () => {
	test("forwards sync chunks in order and completes once with the final checkpoint", async () => {
		...
		const bridge = {
			run: async (options: {
				...
				await options.onStream?.(
					{ items: [item("b")], checkpoint: { cursor: "2" } },
					2,
				);
		...
		expect(
			(client.calls.stream[0] as { items: { id: string }[] }).items.map(
				(entry) => entry.id,
			),
		).toEqual(["a"]);
		expect(
			(client.calls.stream[1] as { items: { id: string }[] }).items.map(
				(entry) => entry.id,
			),
		).toEqual(["b"]);
	...
	test("retries terminal delivery after the first completion request fails", async () => {
		...
		expect(client.calls.completeAction[0]).toEqual(
			client.calls.completeAction[1],
		);
	...
	test("does not advance or commit a checkpoint from a failed stream", async () => {
		...
		const bridge = {
			run: async (options: {
				...
				expect(options.operation).toBe("sync");
				await options.onStream?.(
					{ items: [item("a")], checkpoint: { cursor: "1" } },
					1,
				);
				await options.onStream?.(
					{ items: [item("b")], checkpoint: { cursor: "2" } },
		...
		expect(
			(client.calls.complete[0] as Record<string, unknown>).checkpoint,
		).toBeUndefined();
	});
	...
	test("rejects automation and embed_backfill runs without using completeAction", async () => {
		...
		const embedBackfill = await executeNativeBridgeRun(
			client as never,
			bridge,
			{
				run_id: 18,
		...
		expect(automation.error).toContain(
			"native bridge does not execute run_type automation",
		);
		expect(embedBackfill.error).toContain(
			"native bridge does not execute run_type embed_backfill",
		);
		expect(client.calls.completeAction).toEqual([]);
		expect(client.calls.completeAutomation).toEqual([
			expect.objectContaining({
				runId: 17,
				value: expect.objectContaining({ error: automation.error }),
			}),
		]);
		expect(client.calls.complete).toEqual([
			expect.objectContaining({
				run_id: 18,
				status: "failed",
				error_message: embedBackfill.error,
			}),
		]);
	});
	...
	test("times out a hung native run, cancels its owner, and reports failure", async () => {
		...
		const result = await executeNativeBridgeRun(
			client as never,
			bridge,
			{
				run_id: 19,
