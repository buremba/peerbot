...
## Network and runtime
- Worker subprocesses reach the network through the gateway proxy on `127.0.0.1`, port from `WORKER_PROXY_PORT` (default 8118). Read it from the environment; do not hardcode the port in worker code.
