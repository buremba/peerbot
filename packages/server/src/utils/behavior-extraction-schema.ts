/**
 * Behavior extraction schema — derived from the target entity type (consolidation:
 * "schema lives on the entity type, not the behavior").
 *
 * A behavior that names an `entity_type` in its `keying_config` derives its output
 ...
 * never drift. Returns null when the behavior isn't entity-typed or the type has
...
/**
 * Derive a behavior's extraction schema. Precedence:
 ...
 *     `behaviors.reaction_input_schema` when the behavior is created or its script
 ...
 * so extraction and validation can never drift. `behaviorId` enables the reaction
 * lookup; omit it to skip step 2 (entity-typed-only callers).
 */
export async function deriveBehaviorExtractionSchema(
  sql: DbClient,
  organizationId: string,
  keyingConfig: KeyingConfig | null | undefined,
  behaviorId?: string | number | null
): Promise<Record<string, unknown> | null> {
  ...
  if (behaviorId != null && behaviorId !== '') {
    const rows = await sql<{ reaction_input_schema: Record<string, unknown> | string | null }>`
      SELECT reaction_input_schema FROM behaviors
      WHERE id = ${behaviorId} AND organization_id = ${organizationId}
      LIMIT 1
    `;
